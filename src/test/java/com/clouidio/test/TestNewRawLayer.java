...
import com.clouidio.orm.api.z8spi.meta.DboColumnCommonMeta;
...
public class TestNewRawLayer {
	...
	// In one case, we added this String.long.String a few times and our
	// in-memory version saw different values as being the same
	// due to the Utf8Comparator being not so correct in that it translated some
	// different long values back to the same string utf 8 value
	@Test
	public void testRawComposite() {
		...
		createColumn(row, "5f686973746f72790000013caaa1b98d5f7374617465",
				(byte) 4);
		createColumn(row, "5f686973746f72790000013caaa1b98d6275696c64696e67",
				(byte) 5);
		createColumn(row, "5f686973746f72790000013caaa1b98d6e756d526f6f6d73",
				(byte) 6);
		createColumn(row, "5f686973746f72790000013caaa1b9ba6275696c64696e67",
				(byte) 7);
		createColumn(row, "5f686973746f72790000013caaa1b9ba6e756d526f6f6d73",
				(byte) 8);
	...
	public void testAdvancedJoinClause() {
		...
		Cursor<KeyValue<TypedRow>> cursor = s
				.createQueryCursor(
						"select * FROM User as u left join u.age Child as c on u.age = c.age",
						500).getPrimaryViewCursor();
	...
	public void testBasicChangeToIndex() {
		...
		String id = "someid";
		TypedRow row = s.createTypedRow(cf);
		createUser(row, id, "alex", "barreto");
		s.put(cf, row);
		s.flush();
		...
		Assert.assertEquals(row.getColumn("name").getValue(),
				result.getColumn("name").getValue());
		Assert.assertEquals(row.getColumn("lastName").getValue(), result
				.getColumn("lastName").getValue());
	}
	...
	// 4. save value of 0 length byte array - returns null (use no column to
	// represent null and column with null to represent 0 length)
	// 5. save value of 0 length string - returns null (use no column to
	// represent null and column with null to represent 0 length)
	...
	public void testTimeSeries() {
		...
		Assert.assertEquals(row.getColumn("temp").getValue(),
				result.getColumn("temp").getValue());
		Assert.assertEquals(row.getColumn("someName").getValue(), result
				.getColumn("someName").getValue());

		Cursor<KeyValue<TypedRow>> rowsIter = s.createQueryCursor(
				"select s FROM TimeSeriesData as s where s.key = 25", 500)
				.getPrimaryViewCursor();
		rowsIter.next();
		...
		Assert.assertEquals(row.getColumn("temp").getValue(),
				theRow.getColumn("temp").getValue());

		// Testing a negative value in the SQL here
		Cursor<KeyValue<TypedRow>> rows2 = s.createQueryCursor(
				"select s FROM TimeSeriesData as s where s.key > -25", 500)
				.getPrimaryViewCursor();
		rows2.next();
		...
		Assert.assertEquals(row.getColumn("temp").getValue(), theRow2
				.getColumn("temp").getValue());
	}

	private TypedRow createUser(TypedRow row, String key, String name,
			String lastname) {
		row.setRowKey(key);
	...
	public void testOrmLayerMetaSaved() {
		NoSqlEntityManagerFactory factory = FactorySingleton
				.createFactoryOnce();
		...
		Assert.assertEquals("id", toOne.getFkToColumnFamily().getIdColumnMeta()
				.getColumnName());
	...
	public void testBasic() {
		...
		addMetaClassDbo(metaDb, "OtherEntity", "id", "alex", "pipo", "pet",
				"house");
		...
		String sql = "select * FROM MyEntity as e WHERE e.cat = \"tareco\"";

		TypedRow typedRow = session.createTypedRow("MyEntity");

		typedRow.addColumn("cat", "tareco");
		typedRow.setRowKey("alex1");

		session.put("MyEntity", typedRow);

		session.flush();

		Iterable<KeyValue<TypedRow>> rowsIterable = session.createQueryCursor(
				sql, 50).getPrimaryViewIter();
		int counter = 0;
		for (@SuppressWarnings("unused")
		KeyValue<TypedRow> k : rowsIterable) {
			counter++;
		}
		Assert.assertEquals(1, counter);
	}

	private DboTableMeta addMetaClassDbo(DboDatabaseMeta map,
			String entityName, String idField, String... fields) {
