...
public class TestJoins {
	...
	public void testViewIndex() {
		...
		// Supply the column family(Activity) the column that is
		// indexed(numTimes)
		// IF a table is partitioned one way, you can add a partitionId
		// IF a table is partitioned multiple ways, you MUST supply partitionBy
		// and you can supply a partitionId or null for null partition
		Cursor<IndexPoint> cursor = s.indexView("Activity", "numTimes", null,
				null);
	...
	public void testInnerJoinParts() throws InterruptedException {
		...
		QueryResult result = s.createQueryCursor(
				"select * FROM Activity as e WHERE e.numTimes < 15", 50);
	...
	public void testInnerJoinParts2() throws InterruptedException {
		...
		QueryResult result = s.createQueryCursor(
				"select * FROM Account as a WHERE a.isActive = false", 50);
	...
	/**
	 * This tests the caching of backward iteration. There once was a bug where
	 * it got reversed (back to forward) when iterated backward the second time
	 * because the of the cache.
	 * 
	...
	public void testInnerJoinBackwardRepeatedly() throws InterruptedException {
		...
		Cursor<IndexColumnInfo> cursor = result.getCursor();
		cursor.afterLast();
		// iterate all the way through to force everything to load once, then
		// ensure things are still in the correct (reversed) order:
	...
	// Well, we fail here as postgres returns just two results and does NOT
	// return the results where e.account= null do to the inner join WHERE we do
	// return the
	// results for e.account=null as that is what the user "would" expect since
	// he declared it.
	...
	// "select * FROM Activity as e INNER JOIN e.account as a WHERE e.numTimes < 15 and a.isActive = false"
	// QueryResult result =
	// s.createQueryCursor("select * FROM Activity as e INNER JOIN e.account as a WHERE e.numTimes < 15 and (e.account = null or a.isActive = false)",
	// 50);
	// List<ViewInfo> views = result.getViews();
	// Cursor<IndexColumnInfo> cursor = result.getCursor();
	//
	...
	public void testOuterJoin() throws InterruptedException {
		...
		QueryResult result = s
				.createQueryCursor(
						"select * FROM Activity as e LEFT JOIN e.account as a WHERE e.numTimes < 15 and a.isActive = false",
						50);
		List<ViewInfo> views = result.getViews();
		Cursor<IndexColumnInfo> cursor = result.getCursor();
	...
	public void testOuterJoinWithNullClause() throws InterruptedException {
		...
		QueryResult result = s
				.createQueryCursor(
						"select * FROM Activity as e LEFT JOIN e.account as a WHERE e.numTimes < 15 and (e.account = null or a.isActive = false)",
						50);
		List<ViewInfo> views = result.getViews();
		Cursor<IndexColumnInfo> cursor = result.getCursor();
	...
	private void compareKeys(Cursor<IndexColumnInfo> cursor, ViewInfo viewAct,
			ViewInfo viewAcc, String expectedKey, String expectedAccKey) {
		IndexColumnInfo info = cursor.getCurrent();
		IndexPoint keyForActivity = info.getKeyForView(viewAct);
		String key = keyForActivity.getKeyAsString();
