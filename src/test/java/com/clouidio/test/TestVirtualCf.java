...
import com.clouidio.test.db.PartitionedSingleTrade;
import com.clouidio.test.db.TimeSeriesData;
...
public class TestVirtualCf {
	...
	public void testRawStuff() {
		...
		/*
		 * if (result.getColumn("carOwner") != null) { Object fk =
		 * result.getColumn("carOwner").getValue(); Object rowId =
		 * row2.getRowKey(); //Assert.assertEquals(rowId, fk); } else {
		 */
		// it is saved as a composite column now so no need to test as above,
		// test as composite
		...
		QueryResult qResult = s2.createQueryCursor("select * from MyRaceCar",
				50);
	...
	public void testWriteRead() {
		// let's use TWO entities that share the same columnFamily AND use the
		// same key in both to make sure
		...
		// to really TEST this out, we remove the trade row to make sure we
		// still have the Activity
		// row
		mgr.remove(trade);
		mgr.flush();
		...
		PartitionedSingleTrade r = mgr2.find(PartitionedSingleTrade.class,
				trade.getId());
		Assert.assertNull(r);
