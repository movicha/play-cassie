...
public class TestIndexAndOrParens {
	...
	public void testBooleanWithAndClause() {
		...
		List<Account> activeList = Account.findAnd(mgr, "alex", true);
		printList(" activeList", activeList);
		Assert.assertEquals(2, activeList.size());
		List<Account> reversedList = Account.findAndBackward(mgr, "alex", true);
		printList(" reversedList", reversedList);
		Assert.assertEquals(2, reversedList.size());
		Assert.assertEquals(activeList.get(1).getId(), reversedList.get(0)
				.getId());
		Assert.assertEquals(activeList.get(0).getId(), reversedList.get(1)
				.getId());

		List<Account> nullList = Account.findAnd(mgr, "alex", null);
		Assert.assertEquals(1, nullList.size());

		List<Account> orList = Account.findOr(mgr, "alex", true);
		Assert.assertEquals(5, orList.size());
		reversedList = Account.findOrBackward(mgr, "alex", true);
	...
	public void testSimpleOr() {
		...
		List<Activity> list = Activity.findWithOr(mgr,
				"nothaveThe5OrHellohere", 20);
		Assert.assertEquals(1, list.size());
	}

	@Test
	public void testParensVsNoParens() {
		// @NoSqlQuery(name="findWithParens",
		// query="select * FROM TABLE e WHERE" +
		// " e.name=:name and (e.numTimes = :numTimes or e.isCool = :isCool)"),
		// @NoSqlQuery(name="findWithoutParens",
		// query="select * FROM TABLE e WHERE" +
		// " e.name=:name and e.numTimes = :numTimes or e.isCool = :isCool"),
		// We have a truth table of this where the result of A named query and B
		// named query are on right side
		...
		List<Activity> withParens = Activity.findWithParens(mgr, "notfound",
				99, 5.55f);
		Assert.assertEquals(0, withParens.size());

		// Second query should be found....(B)
		List<Activity> list = Activity.findWithoutParens(mgr, "notfound", 99,
				5.55f);
		Assert.assertEquals(1, list.size());
	}
