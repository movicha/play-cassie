...
import com.clouidio.test.db.PartAccount;
import com.clouidio.test.db.User;

public class TestIndexes {

	private static final Logger log = LoggerFactory
			.getLogger(TestIndexes.class);
	...
	public void testFailureOnTypeMismatch() {
		...
		Query<Activity> query = mgr.createNamedQuery(Activity.class,
				"findBetween");
	...
	public void testFailureOnGetSingleResultAndSuccess() {
		...
		Activity activity = Activity.findSingleResult(mgr,
				act3.getUniqueColumn());
	...
	public void testTwoQueriesSameNameDifferentEntitiesAllowed() {
		// Account has the same name as a query in Activity which IS allowed in
		// our implementation
	...
	public void testTwoQueriesSameNameDifferentEntitiesAllowedBackward() {
		...
		// TODO:JSC this test will fail randomly until we can specify the index
		// to 'find' over
		...
		// Assert.assertEquals(all2.get(0).getId(),
		// allbackward2.get(2).getId());
	...
	public void testIndexedButNotInNoSqlDatabaseList() {
		...
		acc2.setBusinessName("alex");
		mgr.put(acc2);

		mgr.flush();

		// Here we have to go raw and update the index ourselves with another
		// fake PartAccount that does
		// not exist
		...
		session.persistIndex(table, info.getIndexColFamily(), info.getRowKey(),
				col);
		...
		// NOTE: Account3 was NOT PUT in the database(or you could say removed
		// but index not updated yet)
		try {
