...
import com.clouidio.test.db.AccountSuper;
import com.clouidio.test.db.InheritanceSub1;
import com.clouidio.test.db.InheritanceSub2;
import com.clouidio.test.db.InheritanceSuper;
import com.clouidio.test.db.InheritanceToMany;
import com.clouidio.test.db.InheritanceToManySpecific;
import com.clouidio.test.db.InheritanceToOne;
import com.clouidio.test.db.InheritanceToOneSpecific;
...
public class TestInheritanceSingleTable {
	...
	public void testSpecificQuery() {
		...
		Cursor<KeyValue<InheritanceSub1>> results = InheritanceSub1
				.findAll(mgr);
		int size = 0;
	...
	public void testQueryWithParam() {
		...
		InheritanceSub1 result = InheritanceSub1.findByName(mgr,
				common.getName());
		Assert.assertNotNull(result);
		Assert.assertEquals(common.getDiff(), result.getDiff());

		// Now test query on an inherited field
		Query<AccountSuper> query = mgr.createNamedQuery(AccountSuper.class,
				"findByField");
	...
	public void testBasicMultipleClasses() {
		...
		InheritanceSuper many = mgr
				.find(InheritanceSuper.class, toMany.getId());
		Assert.assertTrue(InheritanceSub2.class.isAssignableFrom(many
				.getClass()));
		Assert.assertEquals(toMany.getNum(), many.getNum());
		Assert.assertEquals(toMany.getDescription(),
				((InheritanceSub2) many).getDescription());
	}
	...
	public void testToManyRelatipnshipSpecific() {
		...
		InheritanceToManySpecific result1 = mgr.find(
				InheritanceToManySpecific.class, ent1.getId());
	...
	public void testToOneRelatipnshipSpecific() {
		...
		InheritanceToOneSpecific result1 = mgr.find(
				InheritanceToOneSpecific.class, ent1.getId());
	...
	public void testToOneRelationship() {
		...
		InheritanceToOne result1 = mgr.find(InheritanceToOne.class,
				ent1.getId());
		Assert.assertTrue(result1.getInheritance() instanceof InheritanceSub1);
	...
	public void testToManyRelationship() {
		...
		InheritanceToMany newRel = mgr.find(InheritanceToMany.class,
				rel.getId());
		InheritanceSuper sub1 = newRel.getNameToEntity().get(
				common.getLastName());
		Assert.assertEquals(common.getId(), sub1.getId());
		Assert.assertEquals(common.getName(),
				((InheritanceSub1) sub1).getName());
		Assert.assertEquals(common.getNum(), sub1.getNum());

		InheritanceSuper sub2 = newRel.getNameToEntity().get(
				toMany.getLastName());
		Assert.assertEquals(toMany.getId(), sub2.getId());
		Assert.assertEquals(toMany.getNum(), sub2.getNum());
		Assert.assertEquals(toMany.getDescription(),
				((InheritanceSub2) sub2).getDescription());
	}
}
