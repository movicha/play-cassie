...
public class TestColumnSlice {
	...
	public void testDecimalColumnSlice() throws UnsupportedEncodingException {
		...
		columns.add(new Column(toDecBytes(10.9999999999999999999999999),
				new byte[0]));
		...
		columns.add(new Column(toDecBytes(new BigDecimal(
				"123000111222333444555666.66666666")), new byte[0]));
		columns.add(new Column(toDecBytes(new BigDecimal(
				"-123000111222333444555666.888888")), new byte[0]));
		...
		Cursor<Column> results = session.columnSlice(tableMeta, rowKey, from,
				to, 2);// (scanInfo, from, to, 2);

		int counter = 0;
		while (results.next()) {
			Column col = results.getCurrent();
			if (counter == 0) {
				Assert.assertEquals(-200.23, toDouble(col.getName())
						.doubleValue());
			}
			counter++;
		}
	...
	public void testIntegerColumnSlice() throws UnsupportedEncodingException {
		...
		columns.add(new Column(toIntBytes(-200), new byte[0]));
		columns.add(new Column(toIntBytes(-500), new byte[0]));
		columns.add(new Column(toIntBytes(new BigInteger(
				"123000111222333444555666")), new byte[0]));
		columns.add(new Column(toIntBytes(new BigInteger(
				"-123000111222333444555666")), new byte[0]));
