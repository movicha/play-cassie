...
import com.clouidio.test.db.PartitionedTrade;
...
public class TestPartitioning {
	...
	public void testSinglePartitionedByAnnotationJQL() {
		...
		Query<PartitionedSingleTrade> query1 = mgr.createNamedQuery(
				PartitionedSingleTrade.class, "findByNumber");
		...
		Query<PartitionedSingleTrade> query2 = mgr.createNamedQuery(
				PartitionedSingleTrade.class, "findByNumber");
		query2.setParameter("partitionId", null);
		query2.setParameter("number", 5);
		PartitionedSingleTrade trade3FromNullPartition = query2
				.getSingleObject();
		...
		Query<PartitionedSingleTrade> query3 = mgr.createNamedQuery(
				PartitionedSingleTrade.class, "findBySecurityName");
		query3.setParameter("securityName", "xyz");
		List<PartitionedSingleTrade> tradeFromSecurityPartition = query3
				.getResultList(0, null);
	...
	public void testPartitioning() {
		...
		List<PartitionedTrade> tradesInAcc1Partition = PartitionedTrade
				.findByUnique(mgr, trade.getUniqueColumn(), acc);
