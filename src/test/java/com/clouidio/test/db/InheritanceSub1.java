...
public class InheritanceSub1 extends InheritanceSuper {
	...
	public static Cursor<KeyValue<InheritanceSub1>> findAll(
			NoSqlEntityManager mgr) {
		Query<InheritanceSub1> query = mgr.createNamedQuery(
				InheritanceSub1.class, "findAll");
		return query.getResults("name");
	}

	public static InheritanceSub1 findByName(NoSqlEntityManager mgr, String name) {
		Query<InheritanceSub1> query = mgr.createNamedQuery(
				InheritanceSub1.class, "findByName");
