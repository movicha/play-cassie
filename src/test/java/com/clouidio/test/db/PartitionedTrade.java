...
public class PartitionedTrade {
	...
	public static List<PartitionedTrade> findInPartition(
			NoSqlEntityManager mgr, int shares, String type, Account partitionId) {
		Query<PartitionedTrade> query = mgr.createNamedQuery(
				PartitionedTrade.class, "findJoinOnNullPartition");
	...
	public static List<PartitionedTrade> findLeftOuter(NoSqlEntityManager mgr,
			int shares, String type, Account partitionId) {
		Query<PartitionedTrade> query = mgr.createNamedQuery(
				PartitionedTrade.class, "findLeftOuter");
		query.setParameter("shares", shares);
		query.setParameter("type", type);
		query.setParameter("partId", partitionId);
		return query.getResultList(0, null);
	...
	public static List<PartitionedTrade> findByUnique(NoSqlEntityManager mgr,
		...
		Query<PartitionedTrade> query = mgr.createNamedQuery(
				PartitionedTrade.class, "findByUnique");
