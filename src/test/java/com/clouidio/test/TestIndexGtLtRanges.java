...
import com.clouidio.orm.api.base.NoSqlDao;
...
public class TestIndexGtLtRanges {
	...
	private static final Logger log = LoggerFactory
			.getLogger(TestIndexGtLtRanges.class);
	...
	public void testBasicTableLookup() {
		...
		while (cursor.next()) {
			log.info("column family="
					+ cursor.getCurrent().getValue().getColumnFamily());
			count++;
		}

		// Our table size keeps growing with the number of test cases so just
		// make sure it is larger than 23
		Assert.assertTrue(count > 18);

		Cursor<KeyValue<DboTableMeta>> dboTables = NoSqlDao
				.findTablesWithPrefix(mgr, "Dbo");

		int dboCount = 0;
		while (dboTables.next()) {
			dboCount++;
			log.info("CF="
					+ dboTables.getCurrent().getValue().getColumnFamily());
		}
		Assert.assertEquals(3, dboCount);

		Cursor<KeyValue<DboTableMeta>> dboTables2 = NoSqlDao
				.findTablesWithPrefix(mgr, "DboTable");
		int dboCount2 = 0;
		while (dboTables2.next()) {
			dboCount2++;
			log.info("CF="
					+ dboTables2.getCurrent().getValue().getColumnFamily());
		}
	...
	public void testBasicString() throws InterruptedException {
		...
		if (list.get(0).getNumTimes() != 4) {
			...
			log.info("shutdown now.  failure item1=" + item1.getNumTimes()
					+ "/" + item1.getName() + "  item2=" + item2.getNumTimes()
					+ "/" + item2.getName());
			Thread.sleep(600000);
		}
