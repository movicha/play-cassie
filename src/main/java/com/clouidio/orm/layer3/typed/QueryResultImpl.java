...
public class QueryResultImpl implements QueryResult {
	...
	public Cursor<List<TypedRow>> getAllViewsCursor() {
		directCursor.beforeFirst();
		Cursor<List<TypedRow>> cursor = new CursorAllViews(session, metaQuery,
				directCursor, batchSize);
		return cursor;
	}
	...
	public Cursor<KeyValue<TypedRow>> getPrimaryViewCursor() {
		...
		IndiceCursorProxy indiceCursor = new IndiceCursorProxy(mainView,
				directCursor);

		DboTableMeta meta = mainView.getTableMeta();

		Cursor<KeyValue<TypedRow>> results = session.findAllImpl2(meta, null,
				indiceCursor, metaQuery.getQuery(), batchSize);
	...
	public Iterable<KeyValue<TypedRow>> getPrimaryViewIter() {
		...
		Iterable<KeyValue<TypedRow>> proxy = new IterableProxy<KeyValue<TypedRow>>(
				cursor);
		return proxy;
	}
	...
	public Iterable<List<TypedRow>> getAllViewsIter() {
		...
		Iterable<List<TypedRow>> proxy = new IterableProxy<List<TypedRow>>(
				cursor);
		return proxy;
	}
