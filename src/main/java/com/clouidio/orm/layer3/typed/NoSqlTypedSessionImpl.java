...
import com.clouidio.orm.api.z3api.QueryResult;
import com.clouidio.orm.api.z5api.IndexColumnInfo;
import com.clouidio.orm.api.z5api.IndexPoint;
import com.clouidio.orm.api.z5api.NoSqlSession;
import com.clouidio.orm.api.z5api.QueryParser;
...
import com.clouidio.orm.api.z8spi.meta.NoSqlTypedRowProxy;
...
public class NoSqlTypedSessionImpl implements NoSqlTypedSession {
	...
	/**
	 * NOTE: This must be here so that if you get TypedSession from the
	 * NoSqlEntitManager, he will have the same session object and flush flushes
	 * both typed and ORM data at the same time!!! To be removed eventually
	 * 
	 * @param s
	 */
	...
	public void put(String colFamily, TypedRow typedRow) {
		...
		if (metaClass == null)
			throw new IllegalArgumentException("DboTableMeta for colFamily="
					+ colFamily + " was not found");
		...
		for (IndexData ind : row.getIndexToRemove()) {
			session.removeFromIndex(metaClass, ind.getColumnFamilyName(),
		...
		for (IndexData ind : row.getIndexToAdd()) {
			session.persistIndex(metaClass, ind.getColumnFamilyName(),
					ind.getRowKeyBytes(), ind.getIndexColumn());
		}
	...
	@Override
	public void removeIndexPoint(IndexPoint pt, String partitionBy,
			String partitionId) {
		DboColumnMeta colMeta = pt.getColumnMeta();
		ScanInfo info = ScanInfo.createScanInfo(colMeta, partitionBy,
				partitionId);
	...
	public void addIndexPoint(IndexPoint pt, String partitionBy,
			String partitionId) {
		DboColumnMeta colMeta = pt.getColumnMeta();
		ScanInfo info = ScanInfo.createScanInfo(colMeta, partitionBy,
				partitionId);
	...
	public void remove(String colFamily, TypedRow row) {
		...
		List<IndexData> indexToRemove = metaDbo.findIndexRemoves(
				(NoSqlTypedRowProxy) proxy, byteKey);
	...
	@Override
	public Cursor<KeyValue<TypedRow>> createFindCursor(String colFamily,
			Iterable<Object> keys, int batchSize) {
		if (keys == null)
			throw new IllegalArgumentException("keys list cannot be null");
		DboTableMeta meta = cachedMeta.getMeta(colFamily);
		if (meta == null)
			throw new IllegalArgumentException("Meta for columnfamily="
					+ colFamily + " was not found");
		DboColumnMeta idMeta = meta.getIdColumnMeta();
		DirectCursor<byte[]> noSqlKeys = new TypedProxyWrappingCursor<Object>(
				idMeta, new IterableWrappingCursor<Object>(keys));
		return findAllImpl2(meta, keys, noSqlKeys, null, batchSize);
	}

	<T> AbstractCursor<KeyValue<TypedRow>> findAllImpl2(DboTableMeta meta,
			Iterable<T> keys, DirectCursor<byte[]> noSqlKeys, String query,
			int batchSize) {
		...
		// find one at a time. You would rather have 1 find than 1000 if network
		// latency was 1 ms ;).
		AbstractCursor<KeyValue<Row>> rows2 = session.find(meta, virtKeys,
				true, false, batchSize);
		if (keys != null)
	...
	@Override
	public List<KeyValue<TypedRow>> findAllList(String colFamily,
	...
	public Cursor<IndexPoint> indexView(String columnFamily, String column,
		...
		if (colMeta == null) {
			...
			throw new IllegalArgumentException(
					"Must supply partitionBy parameter BECAUSE this column family="
							+ columnFamily + " is partitioned multiple ways");

		ScanInfo info = ScanInfo.createScanInfo(colMeta, partitionBy,
				partitionId);
		AbstractCursor<IndexColumn> indCol = session.scanIndex(info, null,
				null, null);
		AbstractCursor<IndexPoint> results = new CursorToIndexPoint(
				meta.getIdColumnMeta(), colMeta, indCol);
		return results;
	}

	@Override
	public QueryResult createQueryCursor(String query, int batchSize) {
		SpiMetaQuery metaQuery = noSqlSessionFactory.parseQueryForAdHoc(query,
				mgr);

		SpiQueryAdapter spiQueryAdapter = metaQuery
		...
		DirectCursor<IndexColumnInfo> iter = spiQueryAdapter.getResultList(
				alreadyJoinedViews, null);

		QueryResultImpl impl = new QueryResultImpl(metaQuery, this, iter,
				batchSize);
	...
	private int updateBatch(Cursor<List<TypedRow>> cursor, QueryResult result) {
		...
		if (queryType.equalsIgnoreCase("DELETE")) {
			...
			if (updateList.size() == 0)
				throw new IllegalArgumentException(
						"UPDATE should have some values to set");
			else {
				while (cursor.next()) {
					List<TypedRow> joinedRow = cursor.getCurrent();
	...
	private void updateRow(List<TypedRow> joinedRow,
			List<TypedColumn> updateList) {
	...
	private boolean deleteColumn(List<TypedRow> typeRowList,
			List<TypedColumn> deleteList) {
		for (TypedRow r : typeRowList) {
			ViewInfo view = r.getView();
			...
			for (TypedColumn c : r.getColumnsAsColl()) {
				for (TypedColumn columnforDelete : deleteList) {
					if (columnforDelete.getName().equals(c.getName())) {
						session.removeColumn(metaClass, StandardConverters
								.convertToBytes(r.getRowKey()), c.getNameRaw());
						return true;
					} else if (c.getName().equals(
							columnforDelete.getName()
									+ "."
									+ StandardConverters
											.convertToString(columnforDelete
													.getValue()))) {
						session.removeColumn(metaClass, StandardConverters
								.convertToBytes(r.getRowKey()), c.getNameRaw());
						return true;
	...
	public int count(String columnFamily, String indexedColName, Object value) {
		...
		if (colMeta == null)
			throw new IllegalArgumentException("Column=" + indexedColName
					+ " not found on meta info for column family="
					+ columnFamily);
		else if (!colMeta.isIndexed())
			throw new IllegalArgumentException("Column=" + indexedColName
					+ " is not an indexed column");
		String query = "SELECT * FROM " + columnFamily + " WHERE "
				+ indexedColName + " = ";
