...
public class CursorAllViews extends AbstractCursor<List<TypedRow>> {
	...
	public CursorAllViews(NoSqlTypedSessionImpl session,
			SpiMetaQuery metaQuery, DirectCursor<IndexColumnInfo> directCursor,
			int batchSize) {
	...
	private void createCursors(Map<ViewInfo, TwoLists> map) {
		...
		for (ViewInfo view : eagerlyJoinedViews) {
			...
			DirectCursor<KeyValue<TypedRow>> cursor = session.findAllImpl2(
					meta, null, new IterableWrappingCursor<byte[]>(rowKeys),
					query, batchSize);
			DirectCursor<KeyValue<TypedRow>> fillCursor = new CursorFillNulls(
					cursor, twoLists.getFullKeyList(), view);
		...
		// cachedCursors = new CursorDelayedJoins(cachedCursors,
		// eagerlyJoinedViews);
		// }
	}
