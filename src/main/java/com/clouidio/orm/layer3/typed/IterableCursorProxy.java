...
import com.clouidio.orm.api.z5api.IndexColumnInfo.Wrapper;
...
public class IterableCursorProxy implements Iterable<byte[]> {
	...
	public IterableCursorProxy(ViewInfo view,
			DirectCursor<IndexColumnInfo> indice) {
	...
	private static class IteratorCursorProxy extends AbstractIterator<byte[]> {
		...
		public IteratorCursorProxy(ViewInfo view,
				DirectCursor<IndexColumnInfo> cursor2) {
			this.view = view;
			this.cursor = cursor2;
		}
		...
		public IterHolder<byte[]> nextImpl() {
			...
			if (key == null)
				throw new IllegalArgumentException(
						"key was null, index data seems corrupt on view="
								+ view
								+ " col="
								+ wrapper.getColMeta()
								+ " some value is tied to a null primary key.  Should"
