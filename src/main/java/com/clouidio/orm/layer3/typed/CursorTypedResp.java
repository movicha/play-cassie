...
public class CursorTypedResp<T> extends AbstractCursor<KeyValue<TypedRow>> {
	...
	public CursorTypedResp(DboTableMeta meta2,
			AbstractCursor<KeyValue<Row>> rows2, String query) {
		this.meta = meta2;
	...
	public void afterLast() {
		...
		if (keysIterable != null)
			throw new UnsupportedOperationException(
					"Reverse iteration is not yet supported for this curor type.  Try sending in your keys in reverse order");
	}
	...
	private KeyValue<TypedRow> nextForQuery(KeyValue<Row> kv) {
		...
		if (row == null) {
			...
			keyVal.setKey(key);
			RowNotFoundException exc = new RowNotFoundException(
					"Your query="
							+ query
							+ " contained a value with a pk where that entity no longer exists in the nosql store");
