...
public class CachedMeta {
	...
	private DboTableMeta loadAllTableData(String colFamily) {
		// only synchronize on the column family we need to create so we create
		// it once
		synchronized (colFamily.intern()) {
			...
			counter++;
			if (table == null)
				throw new RuntimeException("table=" + colFamily
						+ " was not found");
			// We don't want lots of threads writing data into this structure as
			// it reads from the database so instead
			// we will prefetch everything that is typically used here....
			DboColumnIdMeta idMeta = table.getIdColumnMeta();
			idMeta.getColumnName(); // make sure the idmeta is loaded from
									// datastore
			// load all columns as well
			for (DboColumnMeta col : table.getAllColumns()) {
				if (col instanceof DboColumnToManyMeta) {
					((DboColumnToManyMeta) col).getFkToColumnFamily()
							.getIdColumnMeta().getColumnName();
				} else if (col instanceof DboColumnToOneMeta) {
					((DboColumnToOneMeta) col).getFkToColumnFamily()
							.getIdColumnMeta().getColumnName();
				}
			}
