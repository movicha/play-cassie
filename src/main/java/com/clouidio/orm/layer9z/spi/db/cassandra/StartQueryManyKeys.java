...
public class StartQueryManyKeys implements StartQueryListener {
	...
	public StartQueryManyKeys(ColumnFamilyHelper columnFamilies, Info info1,
			ScanInfo info, List<byte[]> values, boolean reverse) {
	...
	public List<Future<OperationResult<ColumnList<byte[]>>>> start() {
		...
		for (byte[] val : values) {
			...
			CompositeRangeBuilder range = CassandraSession.setupRangeBuilder(
					from, to, info1, reverse);
			ColumnFamilyQuery query = keyspace.prepareQuery(cf);
			RowQuery<byte[], byte[]> rowQuery = query.getKey(rowKey)
