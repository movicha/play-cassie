...
public class MongoDbSession implements NoSqlRawSession {
	...
	private static final Logger log = LoggerFactory
			.getLogger(MongoDbSession.class);
	...
	public AbstractCursor<IndexColumn> scanIndex(ScanInfo scan, Key from,
		...
		CursorOfIndexes cursor = new CursorOfIndexes(rowKey, batchSize, l,
				indexTableName, from, to);
	...
	public AbstractCursor<KeyValue<Row>> find(DboTableMeta colFamily,
		...
		if (info == null) {
			// If there is no column family in mongodb, then we need to return
			// no rows to the user...
			return new CursorReturnsEmptyRows2(rowKeys);
		}

		CursorKeysToRowsMDB cursor = new CursorKeysToRowsMDB(rowKeys,
	...
	private void persistIndex(PersistIndex action, MetaLookup ormSession) {
		...
		if (indexCfName.equalsIgnoreCase("StringIndice")) {
			keyToPersist = StandardConverters.convertFromBytes(String.class,
					key);
		} else if ((indexCfName.equalsIgnoreCase("IntegerIndice"))) {
			keyToPersist = StandardConverters.convertFromBytes(Integer.class,
					key);
		} else if (indexCfName.equalsIgnoreCase("DecimalIndice")) {
			keyToPersist = StandardConverters.convertFromBytes(Double.class,
					key);
		}
		if (doc != null) {
			// Only edit the row
	...
	private void removeColumns(Remove action, DBCollection table) {
		...
		if (row != null) {
			for (byte[] name : action.getColumns()) {
				row.removeField(StandardConverters.convertFromBytes(
						String.class, name));
			}
		}
	}
	...
	private void removeColumn(RemoveColumn action, MetaLookup ormSession) {
		...
		DBObject row = table.findOne(action.getRowKey());
		if (row != null)
			row.removeField(StandardConverters.convertFromBytes(String.class,
					action.getColumn()));
		// table.remove(row);
	}
	...
	private void persist(Persist action, MetaLookup ormSession) {
		...
		for (Column col : action.getColumns()) {
			...
			if (col.getValue() != null)
				value = col.getValue();
			// doc.append(StandardConverters.convertFromBytes(String.class,
			// col.getName()), value);
	...
	public BasicDBObject findOrCreateRow(DBCollection table, byte[] key) {
		...
		if (row == null) {
			...
			return (BasicDBObject) row;
	}

	public BasicDBObject findIndexRow(DBCollection table, byte[] indexKey,
			byte[] key) {
		BasicDBObject basicRow = new BasicDBObject();
		basicRow.append("i",
				StandardConverters.convertFromBytes(String.class, indexKey));
	...
	public Info createInfo(String dbCollection, ColumnType type,
