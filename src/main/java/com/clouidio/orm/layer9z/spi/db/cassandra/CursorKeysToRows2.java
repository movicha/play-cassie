...
public class CursorKeysToRows2 extends AbstractCursor<KeyValue<Row>> {
	...
	public String toString() {
		...
		if (rowKeys instanceof List)
			keys = "List" + keys;
		String retVal = "CursorKeysToRows2(cassandraFindRows)[" + tabs + keys
				+ tabs + "]";
		StringLocal.set(tabs.length());
		return retVal;
	}

	public void setupMore(Keyspace keyspace, DboTableMeta cf, Info info,
			Cache cache) {
	...
	private void loadCache() {
		...
		if (keysToLookup.size() > 0) {
			...
			resultingRows = new ArrayList<com.netflix.astyanax.model.Row<byte[], byte[]>>()
					.iterator();
		}

		Map<ByteArray, KeyValue<Row>> map = new HashMap<ByteArray, KeyValue<Row>>();
		while (resultingRows.hasNext()) {
			com.netflix.astyanax.model.Row<byte[], byte[]> row = resultingRows
					.next();
