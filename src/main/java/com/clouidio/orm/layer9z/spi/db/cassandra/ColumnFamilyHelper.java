...
import com.clouidio.orm.api.z8spi.ColumnType;
import com.clouidio.orm.api.z8spi.MetaLookup;
import com.clouidio.orm.api.z8spi.SpiConstants;
...
public class ColumnFamilyHelper {
	private static final Logger log = LoggerFactory
			.getLogger(ColumnFamilyHelper.class);
	...
	public void start(Map<String, Object> properties)
		...
		// throw new
		// IllegalArgumentException("The property Bootstrap.HOST was not in the Map or was in the Map but not as a String");
		// else if(keyspaceNameObj == null || !(keyspaceNameObj instanceof
		// String))
		// throw new
		// IllegalArgumentException("The property Bootstrap.KEYSPACE was not in the Map or was in the Map but not as a String");
		// else if(clusterNameObj == null || !(clusterNameObj instanceof
		// String))
		// throw new
		// IllegalArgumentException("The property Bootstrap.CLUSTER_NAME was not in the Map or was in the Map but not as a String");
		if (builderObj == null || !(builderObj instanceof Builder))
			throw new IllegalArgumentException(
					"The property Bootstrap.CASSANDRA_BUILDER was not in the Map or was in Map but was not of type Builder and must be supplied when using Cassandra plugin");
		...
		if (keyspaceName == null)
			throw new IllegalArgumentException(
					"You did not call Builder.forKeyspace on the astyanax Builder api.  We need to know the keyspace to continue");
	...
	private void findExistingColumnFamilies(KeyspaceDefinition keySpaceMeta) {
		List<ColumnFamilyDefinition> cfList = keySpaceMeta
				.getColumnFamilyList();
	...
	private void loadColumnFamilyImpl(ColumnFamilyDefinition def) {
		...
		} else if (formName(IntegerType.class, BytesType.class).equals(
				comparatorType)) {
			type = ColumnType.COMPOSITE_INTEGERPREFIX;
		} else if (formName(DecimalType.class, BytesType.class).equals(
				comparatorType)) {
	...
	private String formName(Class class1, Class class2) {
		return CompositeType.class.getName() + "(" + class1.getName() + ","
				+ class2.getName() + ")";
	}

	@SuppressWarnings("unchecked")
	public Info createInfo(String colFamily, ColumnType type,
			StorageTypeEnum keyType) {
		if (keyType == null)
			return null;
		...
		// should we cache this and just look it up each time or is KISS fine
		// for now....
		ColumnFamily cf = null;
		switch (type) {
		case ANY_EXCEPT_COMPOSITE:
			cf = new ColumnFamily(colFamily, BytesArraySerializer.get(),
					BytesArraySerializer.get());
			break;
		case COMPOSITE_DECIMALPREFIX:
		case COMPOSITE_INTEGERPREFIX:
		case COMPOSITE_STRINGPREFIX:
			com.netflix.astyanax.serializers.AnnotatedCompositeSerializer<GenericComposite> eventSerializer = new AnnotatedCompositeSerializer<GenericComposite>(
					GenericComposite.class);
			cf = new ColumnFamily<byte[], GenericComposite>(colFamily,
					BytesArraySerializer.get(), eventSerializer);
		...
		if (info == null)
			throw new RuntimeException(
					"Could not create and could not find virtual or real colfamily="
							+ virtualCf
							+ " see chained exception AND it could be your name is not allowed as a valid cassandra Column Family name",
					ee);
		return info;
	}
	...
	public Info fetchColumnFamilyInfo(String virtualCf, MetaLookup lookup) {
		...
		// in rare circumstances, there may be a new column family that was
		// created by another server we need to load into
	...
	private Info tryToLoadColumnFamilyVirt(String virtColFamily,
		...
		try {
			...
			if (log.isInfoEnabled())
				log.info("Total time to LOAD column family meta from cassandra="
						+ total);
			return info;
	...
	private Info tryToLoadColumnFamilyImpl(String virtCf, MetaLookup lookup)
		...
		synchronized (virtCf.intern()) {
			log.info("Column family NOT found in-memory=" + virtCf
					+ ", CHECK and LOAD from Cassandra if available");
			...
			if (info != null) {
				log.info("Virt CF=" + virtCf
						+ " already exists and real colfamily=" + realCf
			...
			// perhaps the schema is changing and was caused by someone else,
			// let's wait until it stablizes
			waitForNodesToBeUpToDate(null, 300000);

			// NOW, the schema appears stable, let's get that column family and
			// load it
			KeyspaceDefinition keySpaceMeta = keyspace.describeKeyspace();
			ColumnFamilyDefinition def = keySpaceMeta.getColumnFamily(realCf);
			if (def == null) {
				log.info("Well, we did NOT find any column family=" + realCf
						+ " to load in cassandra(from virt=" + virtCf + ")");
				return null;
			}
			log.info("coooool, we found a new column family="
					+ realCf
					+ "(virt="
					+ virtCf
					+ ") to load so we are going to load that for you so every future operation is FAST");
	...
	private synchronized Exception createColFamily(String virtualCf,
		...
		try {
			...
			long total = System.currentTimeMillis() - start;
			if (log.isInfoEnabled())
				log.info("Total time to CREATE column family in cassandra and wait for all nodes to update="
						+ total);
		} catch (Exception e) {
			log.trace(
					"maybe someone else created at same time, so hold off on throwing exception",
					e);
			return e;
		}
		return null;
	...
	private synchronized void createColFamilyImpl(String virtualCf,
		...
		log.info("CREATING column family=" + virtualCf
				+ " in cassandra keyspace=" + keysp);

		DboTableMeta meta = loadFromInMemoryOrDb(virtualCf, ormSession);
		log.info("CREATING REAL cf=" + meta.getRealColumnFamily()
				+ " (virtual CF=" + meta.getRealVirtual() + ")");
	...
	private void createColFamilyInCassandra(DboTableMeta meta) {
		...
		ColumnFamilyDefinition def = cluster.makeColumnFamilyDefinition()
				.setName(colFamily).setKeyspace(keysp);

		log.info("keyspace=" + def.getKeyspace() + " col family="
				+ def.getName());
	...
	private ColumnFamilyDefinition setColumnNameCompareType(DboTableMeta cf,
		...
		if (type == null) {
			...
			def = def
					.setComparatorType("CompositeType(IntegerType, BytesType)");
		} else if (type == StorageTypeEnum.DECIMAL) {
			def = def
					.setComparatorType("CompositeType(DecimalType, BytesType)");
		} else
			throw new UnsupportedOperationException(
					"Not supported yet, we need a BigDecimal comparator type here for sure");
		return def;
	}
	...
	private ColumnFamilyDefinition addRowKeyValidation(DboTableMeta cf,
		...
		case BYTES:
			break;
		default:
			throw new UnsupportedOperationException("type=" + rowKeyType
					+ " is not supported at this time");
		}
		return def;
	}
	...
	private void addColumnFamily(ColumnFamilyDefinition def) {
		try {
			OperationResult<SchemaChangeResult> result = cluster
					.addColumnFamily(def);
			long timeout = 300000;
	...
	public void waitForNodesToBeUpToDate(
			OperationResult<SchemaChangeResult> result, long timeout)
			throws ConnectionException {
		if (result != null && result.getResult() != null)
			log.info("LOOP until all nodes have same schema version id="
					+ result.getResult().getSchemaId() + " OR timeout in "
					+ timeout + " milliseconds");
		else
			log.info("LOOP until all nodes have same schema version OR timeout in "
					+ timeout + " milliseconds");

		long currentTime = System.currentTimeMillis();
		while (true) {
			Map<String, List<String>> describeSchemaVersions = cluster
					.describeSchemaVersions();
			long now = System.currentTimeMillis();
			if (describeSchemaVersions.size() == 1) {
				String key = describeSchemaVersions.keySet().iterator().next();
				if (result != null && result.getResult() != null
						&& !key.equals(result.getResult().getSchemaId())) {
					log.warn("BUG, in cassandra? id we upgraded schema to="
							+ result.getResult().getSchemaId()
							+ " but the schema on all nodes is now=" + key);
				}
				if (result != null && result.getResult() != null)
					assert result.getResult().getSchemaId() == null
							|| key.equals(result.getResult().getSchemaId()) : "The key and id should be equal!!!! as it is updating to our schema";
				break;
