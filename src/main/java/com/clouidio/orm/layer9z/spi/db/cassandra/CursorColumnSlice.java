...
import com.clouidio.orm.layer9z.spi.db.cassandra.CassandraSession.CreateColumnSliceCallback;
...
class CursorColumnSlice<T> extends AbstractCursor<T> {
	...
	public CursorColumnSlice(CreateColumnSliceCallback l, boolean isComposite,
			BatchListener bListener, Integer batchSize, String logInfo,
			String colName) {
		this.callback = l;
	...
	public void beforeFirst() {
		// FOR some reason in astyanax, we have to keep getting the row query on
		// resets or this Cursor returns 5 rows instead of the
		// SAME two rows it returned the first time.....well, form one of the
		// unit tests
	...
	private Holder<T> buildHolder(com.netflix.astyanax.model.Column<byte[]> col) {
		...
		if (isComposite) {
			...
			com.clouidio.orm.api.z8spi.action.Column c = new com.clouidio.orm.api.z8spi.action.Column();
			byte[] name = (byte[]) obj;
			c.setName(name);
	...
	public static IndexColumn convertToIndexCol(
			com.netflix.astyanax.model.Column<byte[]> col) {
	...
	private void fetchMoreResultsImpl() throws ConnectionException {
		if (subList != null) {
			// If subIterator is not null, we have already previously fetched
			// results!!!
			if (pointer < subList.size() - 1)
				return; // no need to fetch next subiterator since this
						// subIterator has more
			else if (batchSize == null) // then we already fetched everything in
										// first round
				return;
			else if (subList.size() < batchSize) {
				// since we have previous results, we then have a count of those
				// results BUT they
				// did NOT fill up the batch so no need to go to database as we
				// know it has no more results
				return;
			}
		}
