...
import com.clouidio.orm.api.z8spi.action.Action;
import com.clouidio.orm.api.z8spi.action.Column;
import com.clouidio.orm.api.z8spi.action.IndexColumn;
import com.clouidio.orm.api.z8spi.action.Persist;
import com.clouidio.orm.api.z8spi.action.PersistIndex;
import com.clouidio.orm.api.z8spi.action.Remove;
import com.clouidio.orm.api.z8spi.action.RemoveColumn;
import com.clouidio.orm.api.z8spi.action.RemoveIndex;
import com.clouidio.orm.api.z8spi.iter.AbstractCursor;
import com.clouidio.orm.api.z8spi.iter.DirectCursor;
import com.clouidio.orm.api.z8spi.iter.EmptyCursor;
...
public class CassandraSession implements NoSqlRawSession {
	...
	public AbstractCursor<KeyValue<Row>> find(DboTableMeta colFamily,
		...
		Info info = columnFamilies.fetchColumnFamilyInfo(
				colFamily.getColumnFamily(), mgr);
		if (info == null) {
			// If there is no column family in cassandra, then we need to return
			// no rows to the user...
		...
		if (type != ColumnType.ANY_EXCEPT_COMPOSITE) {
			throw new UnsupportedOperationException(
					"Finding on composite type="
							+ colFamily
							+ " not allowed here, you should be using column slice as these rows are HUGE!!!!");
		}

		Keyspace keyspace = columnFamilies.getKeyspace();
		CursorKeysToRows2 cursor = new CursorKeysToRows2(rowKeys, batchSize,
				list, rowProvider);
	...
	private void removeColumn(RemoveColumn action, MetaLookup mgr,
			MutationBatch m) {
		Info info = columnFamilies.fetchColumnFamilyInfo(action.getColFamily()
				.getColumnFamily(), mgr);
		if (info == null)
	...
	private void persistIndex(PersistIndex action, MetaLookup mgr,
			MutationBatch m) {
	...
	private Object createObjectToUse(RemoveIndex action, Info info) {
		...
		default:
			throw new UnsupportedOperationException(
					"not supported at this time. type=" + type);
		}
		return toPersist;
	}

	private void persist(Persist action, MetaLookup ormSession, MutationBatch m) {
		Info info = columnFamilies.lookupOrCreate2(action.getColFamily()
				.getColumnFamily(), ormSession);
		ColumnFamily cf = info.getColumnFamilyObj();
	...
	public AbstractCursor<Column> columnSlice(DboTableMeta colFamily,
			final byte[] rowKey, final byte[] from, final byte[] to,
			final Integer batchSize, BatchListener batchListener, MetaLookup mgr) {
		if (batchSize <= 0)
			throw new IllegalArgumentException(
					"batch size must be supplied and be greater than 0");
		final Info info1 = columnFamilies.fetchColumnFamilyInfo(
				colFamily.getColumnFamily(), mgr);
		if (info1 == null) {
			// well, if column family doesn't exist, then no entities exist
			// either
			log.info("query was run on column family that does not yet exist="
					+ colFamily);
		...
		CreateColumnSliceCallback l = new CreateColumnSliceCallback() {
			...
			public RowQuery<byte[], byte[]> createRowQuery() {
				RangeBuilder rangeBldr = new RangeBuilder().setStart(from)
						.setEnd(to);
				if (batchSize != null)
			...
			public RowQuery<byte[], byte[]> createRowQueryReverse() {
				RangeBuilder rangeBldr = new RangeBuilder().setStart(to)
						.setEnd(from).setReversed(true);
				if (batchSize != null)
					rangeBldr = rangeBldr.setLimit(batchSize);
				ByteBufferRange range = rangeBldr.build();
				return createBasicRowQuery(rowKey, info1, range);
		...
		return findBasic(Column.class, rowKey, l, batchListener, batchSize,
				"general column slice", null);
	}

	@Override
	public AbstractCursor<IndexColumn> scanIndex(ScanInfo info,
			List<byte[]> values, BatchListener batchList, MetaLookup mgr) {
	...
	public AbstractCursor<IndexColumn> scanIndex(ScanInfo info,
		...
		if (type == ColumnType.COMPOSITE_INTEGERPREFIX
			...
			StartQueryListener listener = new StartQueryManyKeys(
					columnFamilies, info1, info, values, reverse);
	...
	public AbstractCursor<IndexColumn> scanIndex(ScanInfo info, Key from,
			Key to, Integer batchSize, BatchListener bListener, MetaLookup mgr) {
		...
		if (type == ColumnType.COMPOSITE_INTEGERPREFIX
			...
			return findBasic(IndexColumn.class, rowKey, l, bListener,
					batchSize, "" + info, colName);
		} else
			throw new UnsupportedOperationException("not done here yet");
	}

	public static CompositeRangeBuilder setupRangeBuilder(Key from, Key to,
			Info info1, boolean reverse) {
		AnnotatedCompositeSerializer serializer = info1
	...
	private RowQuery createBasicRowQuery(byte[] rowKey, Info info1,
			ByteBufferRange range) {
		ColumnFamily cf = info1.getColumnFamilyObj();

		Keyspace keyspace = columnFamilies.getKeyspace();
		ColumnFamilyQuery query = keyspace.prepareQuery(cf);
		// ColumnFamilyQuery query =
		// query1.setConsistencyLevel(ConsistencyLevel.CL_QUORUM);
		RowQuery rowQuery = query.getKey(rowKey).autoPaginate(true)
				.withColumnRange(range);
		return rowQuery;
	}

	private <T> AbstractCursor<T> findBasic(Class<T> clazz, byte[] rowKey,
			CreateColumnSliceCallback l, BatchListener bListener,
			Integer batchSize, String logInfo, String colName) {
	...
	private class Listener implements CreateColumnSliceCallback {
		...
		public Listener(byte[] rowKey, Info info1, Key from, Key to,
				Integer batchSize) {
			this.rowKey = rowKey;
			this.info1 = info1;
			this.from = from;
		...
		/**
		 * For some dang reason with astyanax, we have to recreate the row query
		 * from scratch before we re-use it for a NEsted join.
		 * 
		 * @return
		 */
		public RowQuery<byte[], byte[]> createRowQuery() {
