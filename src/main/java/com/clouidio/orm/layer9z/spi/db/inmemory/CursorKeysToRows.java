...
public class CursorKeysToRows extends AbstractCursor<KeyValue<Row>> {
	...
	public CursorKeysToRows(DboTableMeta colFamily,
			DirectCursor<byte[]> rowKeys, BatchListener list,
			NoSqlDatabase database, Cache cache2) {
	...
	private void loadCache() {
		...
		// into our own List :( :( .....okay, well, we are now in memory and
		// would have been ordered but oh well....rework this later
	...
	public String toString() {
		...
		if (rowKeys instanceof List)
			keys = "List" + keys;
		String retVal = "CursorKeysToRows(inmemoryRowLookup)[" + tabs + keys
				+ tabs + "]";
