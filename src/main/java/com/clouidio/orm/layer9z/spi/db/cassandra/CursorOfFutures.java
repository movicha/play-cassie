...
public class CursorOfFutures extends AbstractCursor<IndexColumn> {
	...
	public Holder<IndexColumn> nextImpl() {
		...
		while (true) {
			if (!theOneBatch.hasNext())
				return null;
			Future<OperationResult<ColumnList<byte[]>>> future = theOneBatch
					.next();
	...
	private OperationResult<ColumnList<byte[]>> get(
			Future<OperationResult<ColumnList<byte[]>>> f) {
		try {
