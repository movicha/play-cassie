package com.clouidio.orm.api.z5api;
...
public interface NoSqlSession {
	...
	public AbstractCursor<KeyValue<Row>> find(DboTableMeta colFamily,
			DirectCursor<byte[]> rowKeys, boolean skipReadCache,
	...
	/**
	 * Returns a special Iterable loads itself first with "batchSize" records
	 * and then after you iterator over those and request "batchSize+1" record,
	 * it hits the database again such that your memory does not explode if you
	 * have a huge amount of records.
	 * 
	 * @param from
	 * @param to
	 * @param batchSize
