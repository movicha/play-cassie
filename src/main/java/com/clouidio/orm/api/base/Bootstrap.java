...
public abstract class Bootstrap {
	...
	public synchronized static NoSqlEntityManagerFactory create(
			Map<String, Object> properties, ClassLoader cl) {
		...
		if ("inmemory".equals(type)) {
			...
			if (clusterName == null || keyspace == null || seeds == null)
				throw new IllegalArgumentException(
						"Must supply the nosql.cassandra.* properties.  Read Bootstrap.java for values");
			createAndAddBestCassandraConfiguration(properties, clusterName,
					keyspace, seeds);

		} else
			throw new IllegalArgumentException("NoSql type=" + type
					+ " not supported. Read Bootstrap.java for possible values");

		return create(dbType, properties, null, cl);
	}

	public synchronized static NoSqlEntityManagerFactory create(
			DbTypeEnum type, Map<String, Object> properties) {
	...
	public synchronized static NoSqlEntityManagerFactory create(
			DbTypeEnum type, String impl, Map<String, Object> properties,
			Map<Class, Converter> converters, ClassLoader cl) {
		Bootstrap newInstance = createInstance(impl);
		NoSqlEntityManagerFactory inst = newInstance.createInstance(type,
				properties, converters, cl);
		return inst;
	}
	...
	protected abstract NoSqlEntityManagerFactory createInstance(
			DbTypeEnum type, Map<String, Object> properties,
			Map<Class, Converter> converters, ClassLoader cl);
	...
	public static void createAndAddBestCassandraConfiguration(
		...
		bootstrap.createBestCassandraConfig(properties, clusterName, keyspace,
				seeds);
	}

	protected abstract void createBestCassandraConfig(
			Map<String, Object> properties, String clusterName,
			String keyspace2, String seeds2);

	public static void createAndAddBestMongoDbConfiguration(
			Map<String, Object> properties, String clusterName,
			String keyspace, String seeds) {
		Bootstrap bootstrap = createInstance(OUR_IMPL);
		bootstrap.createBestMongoDbConfig(properties, clusterName, keyspace,
				seeds);
	}

	protected abstract void createBestMongoDbConfig(
			Map<String, Object> properties, String clusterName,
			String keyspace2, String seeds2);
