...
public class NoSqlDao {
	...
	public static Cursor<KeyValue<DboTableMeta>> findAllTables(
			NoSqlEntityManager mgr) {
		Query<DboTableMeta> query = mgr.createNamedQuery(DboTableMeta.class,
				"findAll");
		return query.getResults();
	}

	public static Cursor<KeyValue<DboTableMeta>> findTablesWithPrefix(
			NoSqlEntityManager mgr, String prefix) {
		Query<DboTableMeta> query = mgr.createNamedQuery(DboTableMeta.class,
				"findLike");
		query.setParameter("prefix", prefix);
		// Just tack on ONE letter that is definitely LARGER than any other
		// character they can supply
