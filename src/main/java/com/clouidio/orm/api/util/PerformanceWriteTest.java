package com.clouidio.orm.api.util;
...
public class PerformanceWriteTest {
	...
	private static final Logger log = LoggerFactory
			.getLogger(PerformanceWriteTest.class);
	...
	public static void main(String[] args) {
		if (args.length != 5) {
			log.warn("Run java com.clouidio.orm.api.base.util.PerformanceWriteTest <host> <clusterName> <timeInSeconds> <numColumnsInTable> <logEveryNthTime>");
			return;
		}
		...
		new PerformanceWriteTest().start(host, clusterName, timeInSeconds,
				numColumns, logEveryNRows);
	}

	private synchronized void finished(int threadNum) {
		numThreadsLeft--;
		log.info("thread num=" + threadNum + " finished.  cnt left="
				+ numThreadsLeft);
		if (numThreadsLeft == 0)
	...
	private void start(String host, String clusterName, int timeInMinutes,
			int numColumns, int logEveryNRows) {
		logEveryN = logEveryNRows;
		// BEFORE Timer, let's get setup first
		Map<String, Object> props = new HashMap<String, Object>();
		props.put(Bootstrap.AUTO_CREATE_KEY, "create");
		Bootstrap.createAndAddBestCassandraConfiguration(props, clusterName,
				"PlayCassiePerfTest", host);
		NoSqlEntityManagerFactory factory = Bootstrap.create(
				DbTypeEnum.CASSANDRA, props, null, null);
	...
	private DboTableMeta setupMetaData(int numColumns,
			NoSqlEntityManagerFactory factory) {
		NoSqlEntityManager mgr = factory.createEntityManager();

		DboDatabaseMeta meta = mgr.find(DboDatabaseMeta.class,
	...
	private class SlamDataIn implements Runnable {
		...
		public SlamDataIn(NoSqlEntityManagerFactory mgrFactory,
				DboTableMeta table, int i) {
		...
		public void run() {
			...
			while (shouldRun) {
				...
				for (int i = 0; i < 1; i++) {
					...
					byte[] key = table.getIdColumnMeta().convertToStorage2(
							rowKey);
