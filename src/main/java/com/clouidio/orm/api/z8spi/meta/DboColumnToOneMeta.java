...
public class DboColumnToOneMeta extends DboColumnMeta {
	...
	public void setup(DboTableMeta owner, String colName,
			DboTableMeta fkToTable, boolean isIndexed, boolean isPartitionedBy) {
	...
	public void translateFromColumn(Row row, TypedRow entity) {
		...
		if (columns != null && !columns.isEmpty()) {
			...
			entity.addColumn(this, fullName, namePrefix, fk, value,
					column.getTimestamp());
		} else {
			...
			if (column == null)
				return;
			byte[] value = column.getValue();
			entity.addColumn(this, getColumnNameAsBytes(), value,
					column.getTimestamp());
		}
	}
