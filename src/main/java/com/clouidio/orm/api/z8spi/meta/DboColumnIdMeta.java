...
public class DboColumnIdMeta extends DboColumnMeta {
	...
	public void setup(DboTableMeta owner, String colName, Class valuesType,
		...
		byte[] asBytes = StandardConverters.convertToBytes(owner
				.getColumnFamily());
		if (asBytes.length > 255)
			throw new IllegalArgumentException(
					"Column family names converted using utf8 must be less than 255 bytes.  your col family="
							+ owner.getColumnFamily() + " is too long");

		this.owner = owner;
		setColumnName(colName);
		// NOTE: We don't call super.setup here BECAUSE super.setup calls
		// owner.addColumn and this is the row key
	...
	public String fetchColumnValueAsString(TypedRow row) {
		throw new UnsupportedOperationException(
				"only used by partitioning and can't partition a primary key column....that is kind of useless");
	}
}
