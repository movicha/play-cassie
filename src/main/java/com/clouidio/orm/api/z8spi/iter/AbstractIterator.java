...
public abstract class AbstractIterator<T> implements Iterator<T> {
	...
	public final boolean hasNext() {
		if (nextValue != null) {
			// DO NOT get nextImpl yet since he has not called next yet to clear
			// out the next value
	...
	public final T next() {
		...
		if (temp == null) {
			// Here, they may call next to get the first element WITHOUT calling
			// hasNext, so we call hasNext
			if (!hasNext())
				throw new IllegalArgumentException(
						"This cursor has run out of value.  Call hasNext() to avoid this exception and check before calling next()!!!!");
			temp = nextValue;
		}
		nextValue = null;
