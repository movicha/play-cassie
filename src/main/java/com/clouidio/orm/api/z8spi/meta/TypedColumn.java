...
public class TypedColumn {
	...
	public TypedColumn(DboColumnToManyMeta colMeta, byte[] prefixName,
			byte[] postFixName, byte[] value, Long timestamp2) {
		this(colMeta, prefixName, value, timestamp2);
		this.subName = postFixName;
	}

	public TypedColumn(DboColumnEmbedSimpleMeta colMeta, byte[] prefixName,
			byte[] postFixName, byte[] value, Long timestamp2) {
		this(colMeta, prefixName, value, timestamp2);
		this.subName = postFixName;
	}

	public TypedColumn(DboColumnToOneMeta colMeta, byte[] prefixName,
			byte[] postFixName, byte[] value, Long timestamp2) {
		this(colMeta, prefixName, value, timestamp2);
		this.subName = postFixName;
	}

	public TypedColumn(DboColumnEmbedMeta colMeta, byte[] prefixName,
			byte[] postFixName, byte[] value, Long timestamp2) {
		this(colMeta, prefixName, value, timestamp2);
		this.subName = postFixName;
	}

	public TypedColumn(DboColumnMeta colMeta, byte[] name, byte[] value,
			Long timestamp2) {
	...
	public String getName() {
		if (columnMeta == null)
			throw new IllegalArgumentException(
					"You need to call getName(Class type) instead as this column is not defined in our schema");

		String strName = StandardConverters
				.convertFromBytes(String.class, name);
		if (!(columnMeta instanceof DboColumnToManyMeta
				|| columnMeta instanceof DboColumnEmbedSimpleMeta
				|| columnMeta instanceof DboColumnToOneMeta || columnMeta instanceof DboColumnEmbedMeta))
			return strName;
		if (columnMeta instanceof DboColumnToManyMeta) {
			DboColumnToManyMeta many = (DboColumnToManyMeta) columnMeta;
			if (subName != null) {
				// Just if the subName is null and use in some other way like in
				// DeleteColumn
	...
	public String getNameAsString(Class<?> type) {
		if (columnMeta != null)
			throw new IllegalArgumentException(
					"This is defined in schema, call getName() instead");
	...
	public void setValueStr(String val) {
		if (columnMeta == null)
			throw new IllegalArgumentException(
					"Call setValueStr(String val, Class type) instead as we don't know the type to store as");
	...
	public void setValueStr(String val, Class<?> typeToStoreAs) {
		if (columnMeta != null)
			throw new IllegalArgumentException(
					"Call setValueStr(String val) instead as we have the type information for this column");

		Object objVal = StandardConverters
				.convertFromString(typeToStoreAs, val);
	...
	public Object getValue() {
		if (columnMeta == null)
			throw new IllegalArgumentException(
					"Call getValue(Class type) instead as this column is not in our schema");
