...
public class DboColumnToManyMeta extends DboColumnMeta {
	...
	/**
	 * This field may be referencing another entity in another table so here is
	 * the meta data on that table as well, but for now, I don't think we need
	 * it until we have joins
	 */
	@NoSqlManyToOne
	private DboTableMeta fkToColumnFamily;

	/**
	 * When someone in an entity uses CursorToMany, it is because there are too
	 * many and could blow out the memory so they use CursorToMany and we store
	 * the tomany in a separate index row so they can cursor over the results.
	 */
	private Boolean isSeparateRow;

	public void setup(DboTableMeta owner, String colName,
			DboTableMeta fkToTable, boolean isCursor) {
	...
	public Class getClassType() {
		throw new UnsupportedOperationException(
				"Need to figure out how to convert Class to a type of Array Class");
		// Class typeInTheArray =
		// fkToColumnFamily.getIdColumnMeta().getClassType();
	}

	@Override
	public StorageTypeEnum getStorageType() {
		StorageTypeEnum typeInTheArray = fkToColumnFamily.getIdColumnMeta()
				.getStorageType();
	...
	private void parseOutKeyList(Row row, TypedRow entity) {
		...
		for (Column col : columns) {
			...
			entity.addColumn(this, fullName, namePrefix, fk, col.getValue(),
					col.getTimestamp());
		}
	}
	...
	private void translateToColumnList(TypedRow entity, RowToPersist row) {
		...
		if (!(valueObj instanceof Collection))
			throw new IllegalArgumentException(
					"For column family="
							+ this.owner.getColumnFamily()
							+ " you passed in a row and column value for column="
							+ getColumnName()
							+ " must inherit from Collection and does not(for this column as it is a ToMany column)");
		Collection fks = (Collection) valueObj;

		Collection<Object> toBeAdded = fks; // all values in the list get added
											// if not an OurAbstractCollection
		Collection<Object> toBeRemoved = new ArrayList<Object>();

		throw new UnsupportedOperationException(
				"not complete yet, need to add the below code but use a more simple list");
		// if(fks instanceof OurAbstractCollection) {
		// OurAbstractCollection<Object> coll =
		// (OurAbstractCollection<Object>)fks;
	...
	private void translateToColumnImpl(Collection<Object> toBeAdded,
			RowToPersist row, Collection<Object> toBeRemoved) {
		// removes first
	...
	public String fetchColumnValueAsString(TypedRow row) {
		throw new UnsupportedOperationException(
				"only used by partitioning and can't partition a multe value column....that is kind of useless");
	}
}
