...
public class TypedRow {
	...
	public void addColumn(DboColumnToManyMeta colMeta, byte[] fullName,
			byte[] namePrefix, byte[] fk, byte[] value, Long timestamp) {
		TypedColumn c = new TypedColumn(colMeta, namePrefix, fk, value,
				timestamp);
		ByteArray b = new ByteArray(fullName);
		columns.put(b, c);
	}

	public void addColumn(DboColumnEmbedSimpleMeta colMeta, byte[] fullName,
			byte[] namePrefix, byte[] fk, byte[] value, Long timestamp) {
		TypedColumn c = new TypedColumn(colMeta, namePrefix, fk, value,
				timestamp);
		ByteArray b = new ByteArray(fullName);
	...
	public void addColumn(DboColumnToOneMeta colMeta, byte[] fullName,
			byte[] namePrefix, byte[] fk, byte[] value, Long timestamp) {
		TypedColumn c = new TypedColumn(colMeta, namePrefix, fk, value,
				timestamp);
		ByteArray b = new ByteArray(fullName);
	...
	public void addColumn(DboColumnEmbedMeta colMeta, byte[] fullName,
			byte[] namePrefix, byte[] fk, byte[] value, Long timestamp) {
		TypedColumn c = new TypedColumn(colMeta, namePrefix, fk, value,
				timestamp);
		ByteArray b = new ByteArray(fullName);
	...
	public void addColumn(DboColumnMeta colMeta, byte[] columnName,
	...
	// }
	// public void addColumn(DboColumnMeta meta, byte[] name, byte[] subName,
	// byte[] value, Long timestamp) {
	// if(meta == null)
	...
	// throw new
	// IllegalArgumentException("Column="+name+" not found on this table AND your value was not byte[].  Use byte[] or pass in a column we know about");
	...
	// if(colMeta == null)
	// throw new
	// IllegalArgumentException("Column="+name+" not found on this table.  Use addColumn passing in byte[] instead if you want to save a column the schema doesn't know about");
