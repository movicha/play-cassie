...
public class DboTableMeta {
	 ...
	 * 
	 * A special case where the table has rows with names that are not Strings.
	 * This is done frequently for indexes like indexes by time for instance
	 * where the name of the column might be a byte[] representing a long value
	 * or an int value In general, this is always a composite type of <indexed
	 * value type>.<primary key type> such that we can do a column scan on the
	 * indexed value type and then get the pk...the pk is part of the name
	 * because otherwise, it would not be unique and would collide with others
	 * that had the same indexed value.
	 */
	private String colNamePrefixType = null;
	/**
	 * This is the type of the column name which is nearly always a String (IT
	 * IS ALWAYS a string when usign the ORM layer).
	 */
	...
	/**
	 * Some virtual column families are embeddable capital Boolean for backwards
	 * compatibility...
	 */
	...
	private static void logClassLoader(String prefix, ClassLoader loader) {
		...
		log.info(prefix
				+ "BEGIN BEGIN classloaders that proxies class exists in="
				+ logMsg + "}");
	...
	public void setup(String virtualCf, String cf, boolean isEmbeddable) {
		if (!NAME_PATTERN.matcher(cf).matches())
			throw new IllegalArgumentException(
					"Table name must match regular expression='[a-zA-Z_][a-zA-Z_0-9\\-]*'");
	...
	public RowToPersist translateToRow(TypedRow typedRow) {
		...
		if (typedRow instanceof NoSqlTypedRowProxy) {
			fieldToValue = ((NoSqlTypedRowProxy) typedRow)
					.__getOriginalValues();
		}

		List<PartitionTypeInfo> partTypes = formPartitionTypesList(typedRow);
		InfoForIndex<TypedRow> info = new InfoForIndex<TypedRow>(typedRow, row,
				getColumnFamily(), fieldToValue, partTypes);
	...
	public List<IndexData> findIndexRemoves(NoSqlTypedRowProxy proxy,
			byte[] rowKey) {
		initCaches();
		...
		InfoForIndex<TypedRow> info = new InfoForIndex<TypedRow>(r, null,
				getColumnFamily(), fieldToValue, partTypes);
	...
	private List<PartitionTypeInfo> formPartitionTypesList(TypedRow row) {
		...
		if (partTypes.size() == 0) {
			// if the table is not partitioned, then we still need to create the
			// one huge partition
	...
	public <T> KeyValue<TypedRow> translateFromRow(Row row) {
		...
		// mark initialized so it doesn't hit the database again and cache
		// original values so if they change
	...
	/**
	 * @param row
	 *            The row from where columns to be filled in the
	 *            <code>TypedRow</code>
	 * @param inst
	 *            The object OR the proxy to be filled in
	 */
	public void fillInInstance(Row row, TypedRow inst) {
		...
		for (Column c : row.getColumns()) {
			byte[] name = c.getName();
			String strName = StandardConverters.convertFromBytes(String.class,
					name);
	...
	public DboColumnMeta getAnyIndex(String indexedColumn,
			DboColumnMeta partColMeta) {
		initCaches();
		if (indexedColumnsCache.size() == 0)
			throw new IllegalArgumentException(
					"The table="
							+ columnFamily
							+ " has no columnes with indexes.  ie. no entity attributes had the @NoSqlIndexed annotation");
		int index = 0;
		...
		if (!colMeta.isIndexed())
			throw new IllegalArgumentException("column=" + indexedColumn
					+ " is not indexed");
		return colMeta;
	}
