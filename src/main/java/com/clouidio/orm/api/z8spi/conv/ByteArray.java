...
public class ByteArray implements Comparable<ByteArray> {
	...
	public int compareTo(ByteArray o) {
		...
		for (int i = 0; i < min; i++) {
			// very annoying but in java, we need the int for the unsigned byte
	...
	// Java has no unsigned byte and we need the unsigned byte value so we can
	// compare..
	...
	public String toString() {
		...
		BigDecimal asDec = StandardConverters.convertFromBytesNoExc(
				BigDecimal.class, key);
		BigInteger asInt = StandardConverters.convertFromBytesNoExc(
				BigInteger.class, key);
