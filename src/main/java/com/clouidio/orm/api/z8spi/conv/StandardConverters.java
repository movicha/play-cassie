...
import com.clouidio.orm.api.z8spi.conv.Converters.BaseConverter;
...
public class StandardConverters {
	...
	public static <T> T convertFromBytes(Class<T> clazz, byte[] data) {
		...
		if (converter == null)
			throw new IllegalArgumentException(
					"Type not supported at this time=" + clazz);
		return (T) converter.convertFromNoSql(data);
	}

	/**
	 * Converts to BigInteger byte form OR BigDecimal byte form OR UTF8 byte
	 * form
	 * 
	 * @param obj
	...
	public static byte[] convertToBytes(Object obj) {
		...
		BaseConverter converter = stdConverters.get(clazz);
		if (converter == null)
			throw new IllegalArgumentException("Type clazz=" + clazz
					+ " is not supported at this time");
		return converter.convertToNoSql(obj);
	}

	/**
	 * Special method as if you convert 5 or 876, they convert to BigInteger
	 * byte form but here we specifically want BigDecimal byte form so we need
	 * to force that.
	 * 
	 * @param obj
	 * @return byte form for the object
	...
	public static byte[] convertToDecimalBytes(Object obj) {
		Object value = obj;
		Class clazz = obj.getClass();
		if (Byte.class.equals(clazz) || Short.class.equals(clazz)
		...
		if (converter == null)
			throw new IllegalArgumentException(
					"Type not supported at this time=" + obj.getClass());
