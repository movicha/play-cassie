...
public class OptimizeAddJoinInfo {
	...
	private void walkTheTree(ParsedNode node) {
		...
		if (node.isAndOrType()) {
			...
			JoinInfo info = new JoinInfo(viewInfo, null, null, null,
					JoinType.NONE);
	...
	private JoinMeta switchChildrenIfNeeded(ParsedNode node, ParsedNode left,
		...
		if (primary.getJoinType() != JoinType.NONE) {
			...
			if (!rightType.contains(primaryTable)) {
				// we need to rewrite the tree so the joining table is on the
				// left(right now, joining is the right side)
				...
				throw new RuntimeException(
						"bug, should never get here.  Both sides should not both contain the primary table unless jointype=NONE");
		}
		return info;
	}
	...
	private void checkNoSidesHavePrimaryView(JoinMeta leftType,
		...
		if (!leftType.contains(primaryTable))
			throw new RuntimeException(
					"bug, should never get here, one side should have primarytable or we can't join");
	}

	private JoinMeta findDirectJoin(ParsedNode left, ParsedNode right) {
		// At this point, we know the left will be a table, but the rightside
		// could be a constant or another table or same table
		...
		if (info == null)
			throw new IllegalArgumentException(
					"Sorry, but you have a and/or clause on alias="
							+ view1.getAlias()
							+ " and alias="
							+ view2.getAlias()
