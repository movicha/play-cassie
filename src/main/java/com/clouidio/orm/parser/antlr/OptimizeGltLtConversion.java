...
public class OptimizeGltLtConversion {
	...
	public void walkTree(ParsedNode node, String query, MetaFacade facade) {
		if (node.getType() != NoSqlLexer.AND && node.getType() != NoSqlLexer.OR)
			return; // We are not interested in other nodes, only AND and OR
					// nodes so we can look at their children ourselves
	...
	private ParsedNode findSidesMatch(ParsedNode node, ChildSide side) {
		...
		// The parent must be an AND so we can try to find another reference to
		// same
	...
	private void delete(ParsedNode first) {
		...
		if (parent == rootNode) {
			// If we are the root node, the tree is now collapsing and removing
			// root node
			rootNode = nodeToMove;
		} else {
	...
	private void processFirstMatch(ParsedNode node, ParsedNode match,
			ChildSide side) {
	...
	public void addExpression(ParsedNode betweenExpr, ParsedNode firstMatch,
			ParsedNode secondMatch) {
		ParsedNode leftAttributeSide = firstMatch.getChild(ChildSide.LEFT);
		String aliasAndColumn = leftAttributeSide.getAliasAndColumn();

		if (firstMatch.getType() == NoSqlLexer.EQ
				|| secondMatch.getType() == NoSqlLexer.EQ) {
			throw new IllegalArgumentException(
					"uhhhmmmm, you are using column="
							+ aliasAndColumn
							+ " twice with 'AND' statement yet one has an = so change to 'OR' or get rid of one. ");
		} else if (firstMatch.getType() == NoSqlLexer.GE
				|| firstMatch.getType() == NoSqlLexer.GT) {
			betweenExpr.setChild(ChildSide.LEFT, firstMatch);
			betweenExpr.setChild(ChildSide.RIGHT, secondMatch);
			if (secondMatch.getType() == NoSqlLexer.GE
					|| secondMatch.getType() == NoSqlLexer.GT)
				throw new IllegalArgumentException(
						"You are using > or >= twice on the same column so delete one of those in your query so we don't have to");
		} else if (firstMatch.getType() == NoSqlLexer.LE
				|| firstMatch.getType() == NoSqlLexer.LT) {
			betweenExpr.setChild(ChildSide.LEFT, secondMatch);
			betweenExpr.setChild(ChildSide.RIGHT, firstMatch);
			if (secondMatch.getType() == NoSqlLexer.LE
					|| secondMatch.getType() == NoSqlLexer.LT)
				throw new IllegalArgumentException(
						"uhmmmm, you are using column="
								+ aliasAndColumn
								+ " twice(which is fine) but both of them are using less than, delete one of them. ");
		} else
			throw new RuntimeException(
					"bug, we should never get here but this should be easy to fix.  ");
	}
}
