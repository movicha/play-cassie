...
public class ParseQueryException extends RuntimeException {
	...
	private static final Logger log = LoggerFactory
			.getLogger(ParseQueryException.class);
	...
	private Tuple init(String message2, RecognitionException cause) {
		...
		try {
			...
			message = "Unexpected token at character position="
					+ characterPosition;
			...
			message = "Bug, Could not form error correctly for this one(turn trace log on to see why)="
					+ cause;
			if (message2 != null)
				message += " " + message2;
		}
