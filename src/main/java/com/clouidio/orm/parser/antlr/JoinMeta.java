...
public class JoinMeta {
	...
	public JoinMeta(JoinInfo info, JoinType type, Set<JoinInfo> set1,
			Set<JoinInfo> set2) {
		this(info, type);
		addInfos(set1);
		addInfos(set2);
	...
	public String toString() {
		return "[" + joinType + ",views=" + views + ", primary="
				+ primaryJoinInfo + "]";
	}
	...
	public JoinMeta fetchJoinMeta(JoinMeta rightSide) {
		...
		// View matches are MUCH less costly than join matches so find view
		// match first!!
		for (ViewInfoImpl infoL : views1) {
			for (ViewInfoImpl infoR : views2) {
				if (infoL == infoR) {
					JoinInfo joinInfo = new JoinInfo(infoL, null, null, null,
							JoinType.NONE);
					Set<JoinInfo> set1 = this.getJoinInfoSet();
					Set<JoinInfo> set2 = rightSide.getJoinInfoSet();
					JoinMeta meta = new JoinMeta(joinInfo, JoinType.NONE, set1,
							set2);
					return meta;
				}
			}
		...
		for (ViewInfoImpl infoL : views1) {
			for (ViewInfoImpl infoR : views2) {
				...
				if (joinInfo != null) {
					Set<JoinInfo> set1 = this.getJoinInfoSet();
					Set<JoinInfo> set2 = rightSide.getJoinInfoSet();
					JoinMeta meta = new JoinMeta(joinInfo,
							joinInfo.getJoinType(), set1, set2);
					return meta;
				}
			}
		}
		...
		throw new IllegalArgumentException(
				"Sorry, if you got here, you have a complex query that is not optimized.  "
						+ "We were trying to find a direct join between infos="
						+ views1
						+ " and infos="
						+ views2
						+ " but could not find a direct"
