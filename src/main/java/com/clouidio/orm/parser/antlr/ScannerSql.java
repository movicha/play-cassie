...
import com.clouidio.orm.api.z8spi.meta.DboColumnIdMeta;
import com.clouidio.orm.api.z8spi.meta.DboColumnMeta;
import com.clouidio.orm.api.z8spi.meta.DboColumnToManyMeta;
import com.clouidio.orm.api.z8spi.meta.DboColumnToOneMeta;
import com.clouidio.orm.api.z8spi.meta.DboTableMeta;
import com.clouidio.orm.api.z8spi.meta.TypeInfo;
...
public class ScannerSql {
	...
	public ExpressionNode compileSql(String query, InfoForWiring wiring,
			MetaFacade facade) {
		...
		ExpressionNode newTree = (ExpressionNode) optimizer.optimize(node,
				facade, query);
	...
	private void validateNoPartitionsMissed(CommonTree theTree,
		...
		if (noAliasTable != null) {
			if (noAliasTable.getTableMeta().getPartitionedColumns().size() > 0)
				throw new IllegalArgumentException(
						"In your from you have a table defined with no alias"
		...
		for (ViewInfo view : wiring.getAllViews()) {
			...
			if (meta.getPartitionedColumns().size() > 0
					&& t.getPartition() == null)
				throw new IllegalArgumentException(
						"You are missing a definition of a partition for alias='"
								+ view.getAlias()
								+ "' since table="
								+ meta.getColumnFamily()
								+ " IS a partitioned table.  For example, you should have PARTITIONS e(:partitionId) SELECT e FROM TABLE as e....");
		}
	}
	...
	private static CommonTree parseTree(String query) {
		...
		try {
			...
			ParseQueryException e = new ParseQueryException(
					" (Check the token after the one the parse complained about.  Are you missing the 'as' keyword",
					re);
			throw e;
		}
	}
	...
	private <T> void walkTheTree(CommonTree tree, InfoForWiring wiring,
		...
		switch (type) {
		// The tree should come in with this order OR we are in BIG TROUBLE as
		// we need the information
		// in the from clause first to get alias and what table the alias maps
		// too
		...
		case NoSqlLexer.WHERE:
			// We should try to get rid of the where token in the grammar so we
			// don't need
	...
	@SuppressWarnings("unchecked")
	private <T> void compileJoinClause(CommonTree tree, InfoForWiring wiring,
			MetaFacade facade) {
		...
		for (CommonTree child : children) {
			...
			if (type == NoSqlLexer.LEFT_OUTER_JOIN) {
				...
				throw new UnsupportedOperationException("bug?, type=" + type
						+ " and we don't process that type for joins");
		}
	}

	@SuppressWarnings("unchecked")
	private <T> void compileJoin(CommonTree tree, InfoForWiring wiring,
			MetaFacade facade, JoinType type) {
		List<CommonTree> children = tree.getChildren();
		...
		if (!(columnMeta instanceof DboColumnToOneMeta))
			throw new IllegalArgumentException(
					"Column="
							+ column
							+ " on table="
							+ tableMeta.getColumnFamily()
							+ " is NOT a OneToOne NOR ManyToOne relationship according to our meta data");
			...
			throw new IllegalArgumentException(
					"Column="
							...
							+ " on table="
							+ tableMeta.getColumnFamily()
							+ " is not indexed.  Add @NoSqlIndex or map/reduce a new index in place and add annotation");
		...
		JoinInfo join = new JoinInfo(tableInfo, columnMeta, existing, colMeta2,
				type);
	...
	private void compileSinglePartition(InfoForWiring wiring, CommonTree child,
		...
		if (info == null)
			throw new IllegalArgumentException(
					"In your PARTITIONS clause, you have an alias='" + alias
							+ "' that is not found in your FROM clause");
		else if (info.getPartition() != null)
			throw new IllegalArgumentException(
					"In your PARTITIONS clause, you define a partition for alias='"
							+ alias + "' twice.  you can only define it once");

		DboColumnMeta partitionColumn;
		DboTableMeta tableMeta = info.getTableMeta();
		List<DboColumnMeta> partitionedColumns = tableMeta
				.getPartitionedColumns();
		if (partitionedColumns.size() == 0)
			throw new IllegalArgumentException(
					"The meta data contains no partitions for table="
							+ tableMeta.getColumnFamily()
							+ " so your alias="
							+ alias
							+ " in your query should not be in the PARTITIONS clause.  Delete it!!!!");
		else if (child.getChildCount() > 1) {
			CommonTree columnNode = (CommonTree) child.getChild(1);
			String nameOfColumn = columnNode.getText();
			String withoutQuotes = nameOfColumn.substring(1,
					nameOfColumn.length() - 1);
			partitionColumn = findColumn(partitionedColumns, withoutQuotes);
			if (partitionColumn == null)
				throw new IllegalArgumentException(
						"The meta data specified column="
								+ nameOfColumn
								+ " of having the "
			...
			throw new IllegalArgumentException(
					"The meta data/Annotations contains MORE than one partition, so you have to specify something like "
							+ alias
							+ "('<columnThatWePartitionedBy>', :partId)");
		else {
	...
	private DboColumnMeta findColumn(List<DboColumnMeta> partitionedColumns,
			String nameOfColumn) {
	...
	@SuppressWarnings({ "unchecked" })
	private <T> void compileFromClause(CommonTree tree, InfoForWiring wiring,
			MetaFacade facade) {
		List<CommonTree> childrenList = tree.getChildren();
	...
	private <T> void loadTableIntoWiringInfo(InfoForWiring wiring,
			CommonTree tableNode, MetaFacade facade) {
		...
		if (tableName.equals("TABLE") && targetTable != null) {
			...
			throw new IllegalArgumentException("Meta data(or Entity)="
					+ tableName + " cannot be found");
		...
		if (tableNode.getChildCount() == 0) {
			if (wiring.getNoAliasTable() != null)
				throw new IllegalArgumentException(
						"This query has two tables with no alias.  This is not allowed");
		...
		if (wiring.getMetaQueryTargetTable() != null)
			throw new RuntimeException(
					"Two tables are not supported at this time.");
	...
	@SuppressWarnings("unchecked")
	private void compileDeleteColumnClause(CommonTree tree,
			InfoForWiring wiring, MetaFacade facade) {
		...
		if (childrenList == null)
			...
			TypeInfo columnTypeInfo = processSide(new ExpressionNode(
					childrenList.get(0)), wiring, null, facade);
			byte[] value = null;
			if (childrenList.size() > 1) {
				// value is only relevant if it is *ToOne or *ToMany
				String withQuotes = childrenList.get(1).getText().trim();
				String withoutQuotes = withQuotes.substring(1,
						withQuotes.length() - 1);
				value = StandardConverters.convertToBytes(withoutQuotes);
			}
			TypedColumn columnforDelete = new TypedColumn(
					columnTypeInfo.getColumnInfo(), columnTypeInfo
							.getColumnInfo().getColumnNameAsBytes(), value,
					null);
	...
	@SuppressWarnings("unchecked")
	private <T> void compileUpdateClause(CommonTree tree, InfoForWiring wiring,
			MetaFacade facade) {
		...
		for (CommonTree child : childrenList) {
			...
			compileExpression(node, wiring, facade);
			TypeInfo columnTypeInfo = processSide(
					node.getChild(ChildSide.LEFT), wiring, null, facade);
			byte[] value = retrieveValue(columnTypeInfo.getColumnInfo(),
					node.getChild(ChildSide.RIGHT));
			TypedColumn columnforUpdate = new TypedColumn(
					columnTypeInfo.getColumnInfo(), columnTypeInfo
							.getColumnInfo().getColumnNameAsBytes(), value,
					null);
	...
	@SuppressWarnings("unchecked")
	private <T> void compileTableClause(CommonTree tree, InfoForWiring wiring,
			MetaFacade facade) {
		List<CommonTree> childrenList = tree.getChildren();
		if (childrenList == null)
			return;
	...
	private <T> void processUpdate(CommonTree tree, InfoForWiring wiring,
			MetaFacade facade) {
	...
	private static <T> void parseSelectResults(CommonTree tree,
		...
		for (CommonTree child : childrenList) {
			...
			case NoSqlLexer.ATTR_NAME:
				...
				if (children == null) // It must be an alias if there are no
										// children!!!
					throw new IllegalArgumentException(
							"You have an alias of="
									+ columnNameOrAlias
									+ " that does not exist in the FROM part of the select statement.  query="
									+ wiring.getQuery());
				if (children.size() > 0) { // we have an alias too!!!
					...
					if (wiring.getInfoFromAlias(alias) == null)
						throw new RuntimeException(
								"The select portion has an attribute="
										+ fullName
										+ " with an alias that was not defined in the from section");
				} else {
					// later made need to add attributeName information to
					// metaQuery here
					...
					if (wiring.getNoAliasTable() == null)
						throw new RuntimeException(
								"The select portion has an attribute="
										+ columnNameOrAlias
										+ " with no alias but there is no table with no alias in from section");
				}
	...
	private static <T> void compileExpression(ExpressionNode node,
		...
		case NoSqlLexer.BETWEEN:
			throw new UnsupportedOperationException(
					"not supported yet, use <= and >= intead for now");
		default:
			break;
		}
	}
	...
	private static void compileComparator(ExpressionNode node,
		...
		// The right side could be value/constant or variable or true or false,
		// or decimal, etc. etc.
		CommonTree leftSide = (CommonTree) expression.getChild(0);
		CommonTree rightSide = (CommonTree) expression.getChild(1);

		// This is a VERY difficult issue. We basically want the type
		// information
		// first either from the constant OR from the column name, then we want
		// to use
		// that NOW or later to verify the type of the other side of the
		// equation, BUT which
		// one has the type information as he needs to go first. If both are
		// parameters
		// that are passed in, we have no type information, correct? :name and
		// :something
		// could be any types and may not match at all so first let's disallow
		// param to param
		// matching since developers can do that BEFORE they run the query
		// anyways in the
		// java code. ie. FIRST, let's find the side with type information

		// screw it, we deleted that code and force you to have one side be a
		// column for now!!!!
		if (isAttribute(rightSide)) {
			...
			throw new IllegalArgumentException(
					"Currently, each param in the where clause must be compared to an attribute.  bad query="
							+ wiring.getQuery() + " bad piece=" + node);
		}
	...
	private static <T> ExpressionNode processSide(ExpressionNode node,
			InfoForWiring wiring, List<CommonTree> children, int i,
			ChildSide side, MetaFacade facade) {
	...
	// if(node.getType() == NoSqlLexer.ATTR_NAME || node.getType() ==
	// NoSqlLexer.DECIMAL
	// || node.getType() == NoSqlLexer.INT_VAL || node.getType() ==
	// NoSqlLexer.STR_VAL)
	// return true;
	// return false;
	// }

	private static TypeInfo processSide(ExpressionNode node,
			InfoForWiring wiring, TypeInfo typeInfo, MetaFacade facade) {
		if (node.getType() == NoSqlLexer.ATTR_NAME) {
			...
			throw new RuntimeException("bug, type not supported yet="
					+ node.getType());
	}

	private static TypeInfo processConstant(ExpressionNode node,
			InfoForWiring wiring, TypeInfo typeInfo) {
		String constant = node.getASTNode().getText();

		StorageTypeEnum ourType;
		if (node.getType() == NoSqlLexer.DECIMAL
				|| node.getType() == NoSqlLexer.DEC_VAL) {
			...
			throw new RuntimeException("bug, not supported type(please fix)="
					+ node.getType());
		...
		if (typeInfo.getColumnInfo() != null) {
			...
			if (constantType != ourType)
				throw new IllegalArgumentException(
						"Types do not match in namedquery=" + wiring.getQuery());
		}
		return null;
	}

	private static void validateTypes(InfoForWiring wiring,
			StorageTypeEnum constantType, TypeInfo typeInfo) {
		...
		if (info instanceof DboColumnToManyMeta)
			throw new IllegalArgumentException("Cannot use column="
					+ info.getColumnName()
					+ " since that is a toMany relationship");
			...
			throw new IllegalArgumentException(
					"Types do not match in namedquery=" + wiring.getQuery()
							+ " for column=" + info.getColumnName() + " type1="
							+ constantType + " type2=" + info.getStorageType());
	}

	private static TypeInfo processColumnName(ExpressionNode attributeNode2,
			InfoForWiring wiring, TypeInfo otherSideType, MetaFacade facade) {
		...
		if (colNameNode.getChildCount() > 0) {
			...
			if (tableInfo == null)
				throw new RuntimeException("The where clause attribute="
						+ textInSql
						+ " has an alias that does not exist in from clause");
		} else {
			tableInfo = wiring.getNoAliasTable();
			if (tableInfo == null)
				throw new RuntimeException(
						"The where clause attribute="
								+ textInSql
								+ " has no alias and from clause only has tables with alias");
		}

		DboTableMeta metaClass = tableInfo.getTableMeta();
		// At this point, we have looked up the metaClass associated with the
		// alias
		DboColumnMeta colMeta = facade.getColumnMeta(metaClass, columnName);
		if (colMeta == null) {
			// okay, there is no column found, but maybe the column name for the
			// id matches(id is a special case)
			colMeta = metaClass.getIdColumnMeta();
			if (!colMeta.getColumnName().equals(columnName)) {
				List<String> names = metaClass.getColumnNameList();
				throw new IllegalArgumentException("There is no column="
						+ columnName + " that exists for table "
						+ metaClass.getColumnFamily()
						+ " potential columns are=" + names + "  rowkey col="
		...
		if (otherSideType.getConstantType() != null) {
			...
			if (!classType.equals(colMeta.getClassType()))
				throw new IllegalArgumentException(
						"Types are not the same for query=" + wiring.getQuery()
								+ " types=" + classType + " and "
								+ colMeta.getClassType());
		}

		return null;
	}

	private static TypeInfo processParam(ExpressionNode parameterNode2,
			InfoForWiring wiring, TypeInfo typeInfo) {
