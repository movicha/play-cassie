...
public class ExpressionNode implements ParsedNode {
	...
	/**
	 * In the case where one has a query of where x.size > 5 and x.size < 10,
	 * and if this is the x.size > 5 node, we add the x.size < 10 node to this
	 * node so we can do range queries for an optimization to narrow the search
	 * down.
	 */
	...
	public String getExpressionAsString(boolean isCalledFromFirstIfBlock) {
		if (isBetweenExpression() || isBetweenExpression()) {
			...
			// This one is a bit tough, as we can't toString on ONE of the two
			// greaterThanExpression or lessThanExpression as that would be
			// OURSELF and
			// we woudl infinitely recurse into the getExpressionAsString
			// function :(
			...
			ExpressionNode greaterThanLeftCol = greaterThan
					.getChild(ChildSide.LEFT);
			ExpressionNode greaterThanRightVar = greaterThan
					.getChild(ChildSide.RIGHT);
			ExpressionNode lessThanRightVar = lessThan
					.getChild(ChildSide.RIGHT);

			String line = greaterThanRightVar + greaterThanSign
					+ greaterThanLeftCol + lessThanSign + lessThanRightVar;
			return line;
		} else if (leftChild != null && rightChild != null) {
			String joinType = fetchJoinType();
			String msg = leftChild.getExpressionAsString(false) + " "
					+ this.commonNode + joinType + " "
					+ rightChild.getExpressionAsString(false);
	...
	public ViewInfoImpl getViewInfo() {
		if (!(state instanceof StateAttribute))
			throw new IllegalStateException("This node is of the wrong type="
					+ this.commonNode.getType());
		StateAttribute attr = (StateAttribute) state;
	...
	public boolean isAndOrType() {
		if (commonNode == null)
			return false;
		if (commonNode.getType() == NoSqlLexer.AND
				|| commonNode.getType() == NoSqlLexer.OR)
			return true;
		return false;
	}
	...
	public void replace(ParsedNode oldChild, ParsedNode newChild) {
		if (oldChild == leftChild) {
			...
			throw new IllegalArgumentException(
					"bug, hmmm, oldChild was not a child of this node, what the?");
	}
