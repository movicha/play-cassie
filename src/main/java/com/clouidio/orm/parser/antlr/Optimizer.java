...
public class Optimizer {
	...
	private ParsedNode optimizeGtLtToBetween(ParsedNode node, String query,
			MetaFacade facade) {
		...
		for (Entry<String, Integer> m : attrs.entrySet()) {
			...
			OptimizeGltLtConversion visitor = new OptimizeGltLtConversion(
					m.getKey());
	...
	// String msg =
	// "this type="+this.getType()+" node type="+attrExpNode.getType();
	// ExpressionNode attributeSideNode = (ExpressionNode)
	// attrExpNode.getChild(ChildSide.LEFT);
	// StateAttribute state2 = (StateAttribute) attributeSideNode.getState();
	// if(attrExpNode.getType() == NoSqlLexer.EQ || this.getType() ==
	// NoSqlLexer.EQ) {
	// throw new
	// IllegalArgumentException("uhhhmmmm, you are using column="+state2.getColumnInfo().getColumnName()
	// +" twice with 'AND' statement yet one has an = so change to 'OR' or get rid of one. "+
	// msg);
	// } else if(attrExpNode.getType() == NoSqlLexer.GE || attrExpNode.getType()
	// == NoSqlLexer.GT) {
	// greaterThanExpression = (ExpressionNode) attrExpNode;
	// lessThanExpression = this;
	// } else if(this.getType() == NoSqlLexer.GE || this.getType() ==
	// NoSqlLexer.GT) {
	// greaterThanExpression = this;
	// lessThanExpression = (ExpressionNode) attrExpNode;
	// } else if(this.getType() == NoSqlLexer.LE || this.getType() ==
	// NoSqlLexer.LT) {
	// throw new
	// IllegalArgumentException("uhmmmm, you are using column="+state2.getColumnInfo()+" twice(which is fine) but both of them are using greater than, delete one of them. "
	// +msg);
	// } else
	// throw new
	// RuntimeException("bug, we should never get here but this should be easy to fix.  "+msg);
}
