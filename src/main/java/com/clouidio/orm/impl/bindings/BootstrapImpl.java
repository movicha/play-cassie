...
public class BootstrapImpl extends Bootstrap {
	...
	public NoSqlEntityManagerFactory createInstance(DbTypeEnum type,
			Map<String, Object> properties, Map<Class, Converter> converters,
			ClassLoader cl2) {
		ClassLoader previous = Thread.currentThread().getContextClassLoader();
		try {
			// Javassit uses the thread's context classloader so we need to set
			// that for javassist then
	...
	private NoSqlEntityManagerFactory createInstanceImpl(DbTypeEnum type,
			Map<String, Object> properties, Map<Class, Converter> converters,
			ClassLoader cl2) {
		...
		Injector injector = Guice.createInjector(new ProductionBindings(type,
				temp));
		NoSqlEntityManagerFactory factory = injector
				.getInstance(NoSqlEntityManagerFactory.class);
		...
		// why not just add setInjector() and setup() in
		// NoSqlEntityManagerFactory
		...
		Builder builder = new AstyanaxContext.Builder()
				...
				.withConnectionPoolConfiguration(
						new ConnectionPoolConfigurationImpl("MyConnectionPool")
								.setMaxConnsPerHost(20).setInitConnsPerHost(2)
								.setSeeds(seeds2).setConnectTimeout(10000))
				.withConnectionPoolMonitor(new CountingConnectionPoolMonitor());

		if (seeds2.contains(",")) {
			// for a multi-node cluster, we want the test suite using quorum on
			// writes and
