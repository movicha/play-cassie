...
import com.clouidio.orm.api.exc.ChildWithNoPkException;
...
import com.clouidio.orm.api.z8spi.meta.InfoForIndex;
...
public final class MetaCursorField<OWNER, PROXY> extends
		MetaAbstractField<OWNER> {
	...
	public void translateFromColumn(Row row, OWNER entity, NoSqlSession session) {
		...
		ScanInfo info = new ScanInfo(ownerMeta.getMetaDbo(), getMetaDbo(),
				indexColFamily, key);
		int batchSize = 200;
		AbstractCursor<IndexColumn> indexCursor = session.scanIndex(info, null,
				null, batchSize);

		CursorProxy<PROXY> cursor = new CursorProxy<PROXY>(entity, session,
				indexCursor, classMeta, batchSize);
	...
	private String formRowKey(byte[] byteKey) {
		...
		// We ALWAYS ignore partition in this case since this index row is
		// ALWAYS tied to this row period...moving it all
		// would be a pain and be useless...
		String rowKey = getMetaDbo().getIndexRowKey(null, null) + "/"
				+ keyAsStr;
		return rowKey;
	}
	...
	public Object fetchField(Object entity) {
		throw new UnsupportedOperationException(
				"only used for partitioning and multivalue column can't partition.  easy to implement if anyone else starts using this though, but for now unsupported");
	}

	@Override
	public String translateToString(Object fieldsValue) {
		throw new UnsupportedOperationException(
				"only used for partitioning and multievalue column can't partition.  easy to implement if anyone else starts using this though, but for now unsupported");
	}
	...
	public void translateToColumn(InfoForIndex<OWNER> info) {
		...
		if (cursor == null)
			return; // just ignore it then since user is not modifying what is
					// in the cursor

		if (!(cursor instanceof CursorToMany))
			throw new IllegalArgumentException(
					"cursor must be of type CursorToMany");

		// NOTE: IF instance of proxy, we can just add AND remove modified items
		// only!!!
		...
		// If it is not our proxy and is brand new cursor, we need to add all of
		// them to the index...
	...
	private void addRemoveItems(InfoForIndex<OWNER> info,
	...
	private IndexData fetchIndexData(InfoForIndex<OWNER> info, PROXY value) {
		RowToPersist row = info.getRow();
		// Value is the Account.java or a Proxy of Account.java field and what
		// we need to save in
		...
		if (byteVal == null && value != null) {
			// if value is not null but we get back a byteVal of null, it means
			// the entity has not been
			// initialized with a key yet, BUT this is required to be able to
			// save this object
			...
			String fieldName = "'" + field.getType().getSimpleName() + " "
					+ field.getName() + "'";
			throw new ChildWithNoPkException(
					"The entity you are saving of type="
							+ owner
							+ " has a field="
							+ fieldName
							...
							+ owner
							+ " OR\n"
							+ "2. Call entityManager.fillInWithKey(Object entity), then SAVE your "
							+ owner
							+ "', then save your "
							+ child
							+ " NOTE that this"
	...
	public void setup(DboTableMeta tableMeta, Field field, String colName,
			MetaAbstractClass<OWNER> ownerMeta,
			MetaAbstractClass<PROXY> classMeta) {
	...
	public String toString() {
		return "MetaCursorField [field='" + field.getDeclaringClass().getName()
				+ "." + field.getName() + "(field type="
				+ field.getType().getName() + "<"
				+ classMeta.getMetaClass().getName() + ">), columnName="
				+ columnName + "]";
	}
	...
	public byte[] translateValue(Object value) {
		throw new UnsupportedOperationException(
				"Bug, this operation shold never be called for lists");
	}
