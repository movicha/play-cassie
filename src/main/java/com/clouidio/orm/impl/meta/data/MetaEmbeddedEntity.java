...
import com.clouidio.orm.api.z8spi.meta.DboColumnEmbedMeta;
...
import com.clouidio.orm.impl.meta.data.collections.ToOneProviderProxy;
...
public class MetaEmbeddedEntity<OWNER, PROXY> extends MetaAbstractField<OWNER> {
			...
			if (name != null) {
				...
				for (Field singleField : fieldsinClass) {
					...
					if (Modifier.isTransient(singleField.getModifiers())
							...
							|| singleField
									.isAnnotationPresent(NoSqlTransient.class))
						continue;
				...
				if (proxy != null) {
					Field[] fieldsinClass = proxy.getClass()
							.getDeclaredFields();
					for (Field singleField : fieldsinClass) {
						singleField.setAccessible(true);
						if (Modifier.isTransient(singleField.getModifiers())
								|| Modifier
										.isStatic(singleField.getModifiers())
								|| singleField
										.isAnnotationPresent(NoSqlTransient.class))
							continue;
	...
	private void addColumn(PROXY proxy, Field singleField, byte[] idValue,
			RowToPersist row) {
	...
	private byte[] formTheColumnName(PROXY p, byte[] id, Field singleField) {
		...
		byte[] singleFieldName = StandardConverters.convertToBytes(singleField
				.getName());
		byte[] columnName = new byte[prefix.length + id.length
				+ singleFieldName.length];
	...
	private Object createProxy(byte[] rowKey, Row row) {
		...
		try {
			...
			throw new UnsupportedOperationException(
					"There is some problem in creating the proxy");
		...
		if (classMeta.getIdField() != null) {
			...
			ReflectionUtil.putFieldValue(newproxy,
					classMeta.getIdField().field, idValue);
			...
			String columnsInEmbeddedRowName = getColumnName()
					+ this.getMetaDbo().convertTypeToString(objVal);
			...
			for (Column colInRow : columnsInRow) {
				...
				Object columnValue = this.getMetaDbo().convertFromStorage2(
						colInRow.getValue());
				if (classMeta.getMetaFieldByCol(null, colName) != null)
					ReflectionUtil.putFieldValue(newproxy, classMeta
							.getMetaFieldByCol(null, colName).getField(),
							columnValue);
			}
		} else {
			...
			for (Column col : columnsWORowKey) {
				...
				Object colVal = this.getMetaDbo().convertFromStorage2(
						embedColumn);
				String columnName = this.getMetaDbo().convertTypeToString(
						colVal);
				Object columnValue = this.getMetaDbo().convertFromStorage2(
						col.getValue());
				if (classMeta.getMetaFieldByCol(null, columnName) != null)
					ReflectionUtil.putFieldValue(newproxy, classMeta
							.getMetaFieldByCol(null, columnName).getField(),
							columnValue);
			}
		}
		return newproxy;
	...
	private List<byte[]> parseColNamePostfix(String columnName, Row row) {
		...
		byte[] namePrefix = StandardConverters
				.convertToBytes(columnNameWithPrefix);
	...
	private void addColumnWithoutId(PROXY proxy, Field singleField,
			RowToPersist row) {
		Object value = ReflectionUtil.fetchFieldValue(proxy, singleField);
		Column c = new Column();
		// byte[] columnName = formTheColumnName(proxy, idValue, singleField);
