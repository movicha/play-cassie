...
public class CursorProxy<T> implements CursorToMany<T> {
	...
	private void loadSomeKeys() {
		...
		while (true) {
			...
			// NOTE: Here the indCol.getPrimaryKey is our owning entities
			// primary key
	...
	public T getCurrent() {
		if (currentProxy == null)
			throw new IllegalArgumentException(
					"There is no element located at this position...check the boolean returned from next before calling getCurrent");

		return currentProxy;
	}

	public T convertIdToProxy(byte[] id, NoSqlSession session) {
		Tuple<T> tuple = proxyMeta.convertIdToProxy(null, null, id,
				new LoadCacheCallback());
	...
	private void loadCache() {
		...
		IndiceToVirtual virtKeys = new IndiceToVirtual(metaDbo,
				new ListWrappingCursor<byte[]>(keyList));
		...
		while (true) {
			...
			Row row = kv.getValue();
			// Tuple<T> tuple = proxyMeta.convertIdToProxy(row, session, key,
			// null);
			if (row != null) {
				T value = proxyList.get(counter);
				proxyMeta.fillInInstance(row, session, value);
				// throw new
				// IllegalStateException("This entity is corrupt(your entity='"+owner+"') and contains a"
				// +
