...
public class MetaIdField<OWNER> extends MetaAbstractField<OWNER> {
	...
	public String toString() {
		return "MetaField [field='" + field.getDeclaringClass().getName() + "."
				+ field.getName() + "(field type=" + field.getType() + ")";
	}
	...
	public void translateToColumn(InfoForIndex<OWNER> info) {
		...
		// NOTICE: there is no call to remove because if an id is changed, it is
		// a new entity and we only need to add index not remove since
		// the old entity was not deleted during this save....we remove from
		// index on remove of old entity only
	}

	@Override
	public Object fetchField(Object entity) {
		throw new UnsupportedOperationException(
				"only used for partitioning and id can't partition.  easy to implement if anyone else starts using this though, but for now unsupported");
	}
		...
		if (!useGenerator) {
			if (id == null)
				throw new IllegalArgumentException(
						"Entity has @NoSqlEntity(usegenerator=false) but this entity has no id="
								+ entity);
			return id;
		} else if (id != null) {
			// to make it easier on users, we now check and read in from
			// database if they have not already

			// OKAY, we definitely do NOT want them setting their own id when
			// autogeneration is used as IF you set the id on TWO entities
			// to the same id and add both of them, you end up with a corrupt
			// index in that you have duplicates of two values pointing to
			// the same exact primary key.
			// if(!(entity instanceof NoSqlProxy))
			// throw new
			// IllegalArgumentException("Uhm, uh, you have useGenerator=true(the default) on @NoSqlId annotation and the entity you "
			// +
	...
	public void setup(DboTableMeta tableMeta, IdInfo info, Field field,
			String columnName, boolean isIndexed) {
