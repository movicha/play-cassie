...
import com.clouidio.orm.impl.meta.data.collections.CacheLoadCallback;

public class NoSqlProxyImpl<T> implements MethodHandler {

	private static final Logger log = LoggerFactory
			.getLogger(NoSqlProxyImpl.class);
	...
	public NoSqlProxyImpl(NoSqlSession session, MetaAbstractClass<T> classMeta,
			Object entityId, CacheLoadCallback cacheLoadCallback) {
		if (classMeta.getColumnFamily() == null)
			throw new IllegalArgumentException(
					"column family in the classMeta parameter cannot be null");
		if (session == null && cacheLoadCallback == null)
			...
			throw new IllegalArgumentException(
					"You must supply a cacheLoadCallback OR a session but NOT both...give us session and we load it, give us cacheLoad and we tell you to load your list of proxies when needed");
	...
	/**
	 * @param selfArg
	 *            - The proxy object(if you call any method on self, it will
	 *            result in calling invoke method AND that includes a simple
	 *            toString like if you did log.info("proxy="+self);
	 * @param superClassMethod
	 *            - The method that is on the superclass like Account.java
	 * @param subclassProxyMethod
	 *            - The method that is on the proxy like Account_$$_javassist_0
	 */
	@SuppressWarnings("unchecked")
	@Override
	public Object invoke(Object selfArg, Method superClassMethod,
			Method subclassProxyMethod, Object[] args) throws Throwable {
		T self = (T) selfArg;
		if (log.isTraceEnabled()) {
			log.trace("name=" + superClassMethod.getName()
					+ "  superClass type="
					+ superClassMethod.getDeclaringClass());
			log.trace("name=" + subclassProxyMethod.getName() + " proxy type="
					+ subclassProxyMethod.getDeclaringClass());
		}

		// Here we shortcut as we do not need to go to the database...
		if (idMethod.equals(superClassMethod))
			return entityId;
		else if ("__markInitializedAndCacheIndexedValues"
				.equals(superClassMethod.getName())) {
		...
		// Any other method that is called, toString, getHashCode, getName,
		// someMethod() all end up
		// loading the objects fields from the database in case those methods
		// use those fields
		if (!isInitialized) {
			// If we have a cacheLoadCallback from a List or Map, we are not
			// just loading
			// this entity but the callback method will load this and all other
			// entities from
		...
		return subclassProxyMethod.invoke(self, args); // execute the original
														// method.
	}
	...
	private void fillInThisOneInstance(T self) {
		...
		AbstractCursor<KeyValue<Row>> rows = session.find(metaDbo,
				new IterableWrappingCursor<byte[]>(rowKeys), false, true, null);
		Holder<KeyValue<Row>> holder = rows.nextImpl();
		if (holder == null)
			throw new RowNotFoundException("row for type="
					+ classMeta.getMetaClass().getName()
					+ " not found for key=" + entityId);
