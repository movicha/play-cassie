...
public class MetaInfo {
	...
	public MetaAbstractClass<?> findOrCreate(Class clazz) {
		...
		if (log.isDebugEnabled())
			log.debug("Adding mapping clazz=" + clazz + " to type="
					+ metaClass2.getClass().getSimpleName());
	...
	public Collection<MetaAbstractClass> getAllEntities() {
		// The random order every time we start is very annoying to me and to
		// users. Let's order the values every
		// time instead
