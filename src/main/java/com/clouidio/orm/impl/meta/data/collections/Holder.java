...
/**
 * A class specifically so we can account for nulls. ie. if we don't use holders
 * and we get element[8] and it returns null, we don't know if it cached null or
 * if we need to reprocess. If we use Holder, element[8] returning null means we
 * have not cached it yet, otherwise it can return a Holder with a null value
 * inside it meaning we cached the null value.
 * 
 * @author Alex Barreto
 * 
 * @param <T>
...
public class Holder<T> {
	...
	public Holder(MetaAbstractClass<T> metaClass, NoSqlSession session,
			byte[] nonVirtKey, CacheLoadCallback cb) {
		if (session != null && cb != null)
			throw new IllegalArgumentException(
					"provide session OR cb but not both");
	...
	public synchronized T getValue() {
		if (!hasValue) {
			...
			Tuple<T> tuple = metaClass.convertIdToProxy(null, session,
					key.getKey(), cacheLoadCallback);
