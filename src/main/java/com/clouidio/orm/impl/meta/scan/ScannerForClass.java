...
import com.clouidio.orm.api.base.anno.NoSqlTransient;
...
public class ScannerForClass {
	...
	private static final Logger log = LoggerFactory
			.getLogger(ScannerForClass.class);
	...
	public void addClass(Class<?> clazz) {
		...
		// NOTE: We scan linearly with NO recursion BUT when we hit an object
		// like Activity.java
		// that has a reference to Account.java and so the MetaClass of Activity
		// has fields and that
		// field needs a reference to the MetaClass of Account. To solve this,
		// it creates a shell
		// of MetaClass that will be filled in here when Account gets scanned(if
		// it gets scanned
		// after Activity that is). You can open call heirarchy on
		// findOrCreateMetaClass ;).
		MetaAbstractClass classMeta = metaInfo.findOrCreate(clazz);

		NoSqlInheritance annotation = clazz
		...
		if (classMeta.getIdField() == null && !metaDbo.isEmbeddable()
			...
			throw new IllegalArgumentException(
					"Entity="
							+ classMeta.getMetaClass()
							+ " has no field annotated with @NoSqlId and that is required");
	...
	private <T> void scanMultipleClasses(NoSqlInheritance annotation,
			MetaClassInheritance<T> metaClass) {
		...
		for (Class clazz : annotation.subclassesToScan()) {
			MetaClassSingle<?> metaSingle = metaClass.findOrCreate(clazz,
					mainClass);
	...
	private <T> void scanSingle(MetaClassSingle<T> classMeta,
			DboTableMeta metaDbo) {
	...
	/**
	 * An early test so we get errors on startup instead of waiting until
	 * runtime(a.k.a fail as fast as we can)
	 */
	private Proxy testInstanceCreation(Class<?> clazz) {
		try {
			...
			throw new RuntimeException(
					"ARE YOU missing a default constructor on this class.  We Could not create proxy for type="
							+ clazz, e);
	...
	private void scanForAnnotations(MetaAbstractClass<?> meta) {
		NoSqlEntity noSqlEntity = meta.getMetaClass().getAnnotation(
				NoSqlEntity.class);
		NoSqlEmbeddable embeddable = meta.getMetaClass().getAnnotation(
				NoSqlEmbeddable.class);
		String cf = null;
		...
		if (noSqlEntity != null) {
			...
			if (embeddable != null)
				throw new IllegalArgumentException(
						"You can't have NoSqlEntity AND be NoSqlEmbeddable.  remove one of the annotations");
			...
			if (cf == null)
				cf = meta.getMetaClass().getSimpleName();
			// throw new
			// RuntimeException("bug, someone added an annotation but didn't add to this else clause(add else if to this guy)");
		}

		NoSqlVirtualCf anno = meta.getMetaClass().getAnnotation(
				NoSqlVirtualCf.class);
		if (anno != null) {
	...
	private void inspectField(MetaClassSingle<?> metaClass,
			DboTableMeta metaDbo, Field field) {
		try {
			inspectFieldImpl(metaClass, metaDbo, field);
		} catch (RuntimeException e) {
			throw new RuntimeException("Failure scanning field=" + field
					+ " for class=" + metaClass.getMetaClass().getSimpleName(),
					e);
		}
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void inspectFieldImpl(MetaClassSingle<?> metaClass,
			DboTableMeta metaDbo, Field field) {
		...
		if (field.isAnnotationPresent(NoSqlManyToOne.class))
			...
			metaField = inspectorField.processManyToMany(metaClass, metaDbo,
					field);
		else if (field.isAnnotationPresent(NoSqlOneToMany.class))
			metaField = inspectorField.processOneToMany(metaClass, metaDbo,
					field);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private boolean processIdFieldWorks(MetaAbstractClass metaClass,
			DboTableMeta metaDbo, Field field) {
		...
		if (metaClass.getIdField() != null) {
			...
			if (field.equals(existingField)) {
				log.warn(
						"We avoided double scanning a class="
								+ metaClass.getClass()
								+ " Everything will still work fine, but please send us the stack trace so we can see why this is happening",
						new RuntimeException().fillInStackTrace());
				log.warn("The first entry into this method was=",
						metaClass.getFirstTrace());
				return true; // we already processed it
			} else
				throw new IllegalArgumentException(
						"class="
								+ metaClass.getClass()
								+ " has two fields that have @NoSqlId annotation.  One of them may be in a superclass.  The two fields are="
								+ field + " and=" + existingField);
		} else if (log.isDebugEnabled()) {
			metaClass
					.setFirstTrace(new RuntimeException(
							"first trace of how we set the id field=" + field
									+ " for metaClass=" + metaClass)
							.fillInStackTrace());
		}

		MetaIdField idField = inspectorField.processId(metaDbo, field,
				metaClass);
