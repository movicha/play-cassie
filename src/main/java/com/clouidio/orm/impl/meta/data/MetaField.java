...
public interface MetaField<OWNER> {
	...
	/**
	 * For when we are translating from nosql row, this is called on each
	 * field/column to translate
	 */
	public void translateFromColumn(Row column, OWNER entity,
			NoSqlSession session);

	/**
	 * For when we are translating TO nosql row, this is called on each
	 * field/column
	 */
	...
	/**
	 * For when Query.setParameter is called, we need to translate into byte[]
	 * to search the index for the value
	 * 
	 * @param value
