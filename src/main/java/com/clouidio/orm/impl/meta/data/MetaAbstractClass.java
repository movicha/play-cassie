...
public abstract class MetaAbstractClass<T> implements MetaClass<T> {
	...
	// This is a dynamic class using NoSqlProxyImpl.java as the
	...
	public SpiMetaQuery getNamedQuery(Class<? extends T> clazz, String name) {
		...
		if (query == null)
			throw new IllegalArgumentException("Named query=" + name
					+ " does not exist on type=" + getMetaClass().getName());
		return query;
	}
	...
	public abstract Tuple<T> convertIdToProxy(Row row, NoSqlSession session,
