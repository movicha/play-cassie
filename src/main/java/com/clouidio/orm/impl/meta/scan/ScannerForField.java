package com.clouidio.orm.impl.meta.scan;
...
import com.clouidio.orm.api.base.anno.NoSqlColumn;
import com.clouidio.orm.api.base.anno.NoSqlConverter;
...
import com.clouidio.orm.api.base.anno.NoSqlOneToOne;
import com.clouidio.orm.api.base.anno.NoSqlPartitionByThisField;
import com.clouidio.orm.api.base.spi.KeyGenerator;
import com.clouidio.orm.api.z5api.NoConversion;
import com.clouidio.orm.api.z8spi.conv.Converter;
import com.clouidio.orm.api.z8spi.conv.StandardConverters;
import com.clouidio.orm.api.z8spi.meta.DboTableMeta;
import com.clouidio.orm.api.z8spi.meta.ReflectionUtil;
import com.clouidio.orm.impl.meta.data.IdInfo;
import com.clouidio.orm.impl.meta.data.MetaAbstractClass;
import com.clouidio.orm.impl.meta.data.MetaClassInheritance;
import com.clouidio.orm.impl.meta.data.MetaClassSingle;
import com.clouidio.orm.impl.meta.data.MetaCommonField;
import com.clouidio.orm.impl.meta.data.MetaCursorField;
import com.clouidio.orm.impl.meta.data.MetaEmbeddedEntity;
import com.clouidio.orm.impl.meta.data.MetaEmbeddedSimple;
import com.clouidio.orm.impl.meta.data.MetaField;
import com.clouidio.orm.impl.meta.data.MetaIdField;
import com.clouidio.orm.impl.meta.data.MetaInfo;
import com.clouidio.orm.impl.meta.data.MetaProxyField;
import com.clouidio.orm.impl.meta.data.MetaToManyField;

@SuppressWarnings("rawtypes")
public class ScannerForField {
	private static final Logger log = LoggerFactory
			.getLogger(ScannerForField.class);
	...
	public <T> MetaIdField<T> processId(DboTableMeta t, Field field,
			MetaAbstractClass<T> metaClass) {
		...
		if (field.isAnnotationPresent(NoSqlPartitionByThisField.class))
			throw new IllegalArgumentException("Field=" + field
					+ " is a primary key so it cannot have annotation="
					+ NoSqlPartitionByThisField.class.getName());
	...
	private IllegalArgumentException throwInvalidConverter(Field field) {
		...
		if (Date.class.equals(type) || Calendar.class.equals(type))
			return new IllegalArgumentException(
					"See this url for what you did wrong: https://clouidio.github.com/wiki/play-cassie-documentation/Date-and-Calendar-Support");

		// TODO: create url with information on this one..
		return new IllegalArgumentException(
				"No converter found for field='"
						+ field.getName()
						+ "' in class="
						+ field.getDeclaringClass()
						+ ".  You need to either add one of the @*ToOne annotations, @Embedded, @Transient "
		...
		try {
			...
			if (!method.getReturnType().equals(field.getType()))
				throw new IllegalArgumentException("The method="
						+ declaringClass.getName() + "." + methodName + " must"
						+ " return the type=" + field.getType().getName()
						+ " but instead returns="
						+ method.getReturnType().getName());

			return method;
		} catch (SecurityException e) {
			throw new RuntimeException("security issue on looking up method="
					+ declaringClass.getName() + "." + methodName, e);
	...
	public MetaField processColumn(DboTableMeta t, Field field) {
		...
		Converter converter = null;
		if (col != null
				&& !NoConversion.class.isAssignableFrom(col.customConverter()))
	...
	public MetaField processManyToMany(MetaClassSingle<?> metaClass,
			DboTableMeta t, Field field) {
		NoSqlManyToMany annotation = field.getAnnotation(NoSqlManyToMany.class);
		String colName = annotation.columnName();
		String keyFieldForMap = annotation.keyFieldForMap();

		return processToManyRelationship(metaClass, t, field, colName,
				keyFieldForMap);
	}

	public MetaField processOneToMany(MetaClassSingle<?> ownerMeta,
			DboTableMeta t, Field field) {
		NoSqlOneToMany annotation = field.getAnnotation(NoSqlOneToMany.class);
		String colName = annotation.columnName();
		String keyFieldForMap = annotation.keyFieldForMap();

		return processToManyRelationship(ownerMeta, t, field, colName,
				keyFieldForMap);
	}

	private MetaField processToManyRelationship(MetaClassSingle<?> metaClass,
			DboTableMeta t, Field field, String colNameOrig,
		...
		if (field.isAnnotationPresent(NoSqlPartitionByThisField.class))
			throw new IllegalArgumentException("Field=" + field
					+ " is ToMany annotation so it cannot have annotation="
					+ NoSqlPartitionByThisField.class.getName());
		...
		if (field.getType().equals(Map.class)) {
			if ("".equals(keyFieldForMap))
				throw new RuntimeException(
						"Field="
								+ field
								+ " is a Map so @OneToMany annotation REQUIRES a keyFieldForMap attribute which is the field name in the child entity to use as the key");
			...
			try {
				...
				throw new RuntimeException("The annotation OneToMany on field="
						+ field + " references a field=" + fieldName
						+ " that does not exist on entity="
						+ entityType.getName());
			} catch (SecurityException e) {
		...
		return processToMany(metaClass, t, field, colName, entityType,
				fieldForKey);
	}

	@SuppressWarnings("unchecked")
	private MetaField processToMany(MetaClassSingle<?> ownerMeta,
			DboTableMeta t, Field field, String colName, Class entityType,
			Field fieldForKey) {
		// at this point we only need to verify that
		// the class referred has the @NoSqlEntity tag so it is picked up by
		...
		if (!entityType.isAnnotationPresent(NoSqlEntity.class)) {
			if (!entityType.isAnnotationPresent(NoSqlDiscriminatorColumn.class))
				throw new RuntimeException(
						"type="
								+ entityType
								+ " needs the NoSqlEntity annotation(or a NoSqlDiscriminatorColumn if it is a subclass of an entity)"
								+ " since field has *ToOne annotation.  field="
			...
			// throw new
			// RuntimeException("You have entityType="+entityType.getName()+" so that class needs the NoSqlEntity annotation"
			// +
		...
		if (theSuperclass != null) {
			// we need to swap the classMeta to the more specific class meta
			// which may have not been
			// created yet, oh joy...so we findOrCreate and the shell will be
		...
		if (!field.getType().equals(Map.class)
			...
			throw new RuntimeException(
					"field="
							+ field
							+ " must be Set, Collection, List or Map since it is annotated with OneToMany");
	...
	public MetaField processEmbedded(DboTableMeta t, Field field) {
		...
		if (type.equals(List.class)) {
			ParameterizedType genType = (ParameterizedType) field
					.getGenericType();
		...
		if (type.isAnnotationPresent(NoSqlEmbeddable.class)) {
			...
			if (converter == null)
				throw new IllegalArgumentException(
						"We found no converters(customer or standard for type="
								+ type.getSimpleName()
								+ " and this class is not annotated with "
								+ "@NoSqlEmbeddable either.  The field that caused this issue is field="
								+ field);
	...
	@SuppressWarnings("unchecked")
	public MetaField processToOne(DboTableMeta t, Field field,
			String colNameOrig) {
		String colName = field.getName();
		if (!"".equals(colNameOrig))
			colName = colNameOrig;
		...
		if (!field.getType().isAnnotationPresent(NoSqlEntity.class)
			...
			if (!field.getType().isAnnotationPresent(
					NoSqlDiscriminatorColumn.class))
				throw new RuntimeException(
						"type="
								+ field.getType()
								+ " needs the NoSqlEntity annotation(or a NoSqlDiscriminatorColumn if it is a subclass of an entity)"
			...
			throw new IllegalArgumentException(
					"Okay, so here is the deal.  You have a ToOne relationship defined with an "
							...
							+ "need to do is change this Field="
							+ field
							+ " to='private ToOneProvider<YourType> provider = new ToOneProvider<YourType>() "
		...
		metaField.setup(t, field, colName, classMeta, isIndexed,
				isPartitionedBy);
		return metaField;
	}

	private void runOtherSuperclassChecks(Class entityType, Field field,
			Class<?> theSuperclass) {
		...
		if (theSuperclass == null)
			throw new RuntimeException(
					"type="
							+ entityType
							+ " has a NoSqlDiscriminatorColumn but as we go "
							+ "up the superclass tree, none of the classes are annotated with NoSqlEntity, please add that annotation");
		NoSqlInheritance anno = theSuperclass
				.getAnnotation(NoSqlInheritance.class);
		if (anno == null)
			throw new RuntimeException(
					"type="
							+ entityType
			...
			throw new RuntimeException(
					"type="
							+ entityType
							+ " has a NoSqlDiscriminatorColumn and has a super class with NoSqlEntity and NoSqlInheritance but is not listed"
