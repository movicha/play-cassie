...
public final class MapProxyFetchAll<K, V> extends HashMap<K, V> implements
		CacheLoadCallback {

	private static final Logger log = LoggerFactory
			.getLogger(MapProxyFetchAll.class);
	...
	public static MapProxyFetchAll create(Object owner, NoSqlSession session,
			MetaAbstractClass classMeta, List<byte[]> keys, Field fieldForKey,
			Field field) {
		return new MapProxyFetchAll(owner, session, classMeta, keys,
				fieldForKey, field);
	}

	private MapProxyFetchAll(Object owner, NoSqlSession session,
			MetaAbstractClass<V> classMeta, List<byte[]> keys,
			Field fieldForKey, Field field) {
	...
	// on a hit of getXXXXX (except for getId which doesn't need to go to
	// database)
	...
	public void loadCacheIfNeeded() {
		...
		IndiceToVirtual virtKeys = new IndiceToVirtual(metaDbo,
				new ListWrappingCursor<byte[]>(keys));
		AbstractCursor<KeyValue<Row>> rows = session.find(metaDbo, virtKeys,
				false, true, null);
		String name = getClass().getSimpleName();

		log.info(name
				+ ":just loaded rows for keylist(next convert to proxies)="
				+ keys.size() + " for field=" + field);
		while (true) {
			...
			Row row = kv.getValue();
			Tuple<V> tuple = classMeta.convertIdToProxy(row, session,
					nonVirtKey, null);
			if (row == null)
				continue;
	...
	@Override
	public Set<K> keySet() {
		// Well, we don't know the keys since we have not loaded from cache as
		// the
		// key could be any field and we only have ids in memory before cache is
		// loaded
	...
	public Collection<V> getToBeRemoved() {
		...
		// If remove all(clear method called) we still need to check in case
		// they added some
