...
import com.clouidio.orm.api.z8spi.meta.PartitionTypeInfo;
...
public class MetaClassSingle<T> extends MetaAbstractClass<T> {
	...
	/**
	 * This is NOT fieldName to MetaField but columnName to field(columnName can
	 * be different than fieldname)
	 */
	 ...
	 * @param row
	 * @param session
	 *            - The session to pass to newly created proxy objects
	 * @param inst
	...
	public RowToPersist translateToRow(T entity) {
		...
		InfoForIndex<T> info = new InfoForIndex<T>(entity, row,
				getColumnFamily(), fieldToValue, partitions);
		...
		for (MetaField<T> m : columnNameToField.values()) {
			try {
				...
				throw new TypeMismatchException(
						"The entity "
								+ getMetaClass()
								+ " has an incorrect annotation on field="
								+ m.getField().getName()
								+ ".  The attribute 'entityType' on the annotation is incorrect for the types you are saving.",
						e);
			}
		}
		return row;
	...
	private List<PartitionTypeInfo> formPartitionTypes(Object entity) {
		...
		if (partInfo.size() == 0) {
			// This table is not partitioned so we still need the default null,
			// null partTypeInfo so indexes will process correctly
	...
	public List<IndexData> findIndexRemoves(NoSqlProxy proxy, byte[] rowKey) {
		...
		InfoForIndex<T> info = new InfoForIndex<T>((T) proxy, null,
	...
	@Override
	public Tuple<T> convertIdToProxy(Row row, NoSqlSession session,
			byte[] nonVirtKey, CacheLoadCallback cacheLoadCallback) {
		...
		T proxy = idField
				.convertIdToProxy(session, entityId, cacheLoadCallback);
