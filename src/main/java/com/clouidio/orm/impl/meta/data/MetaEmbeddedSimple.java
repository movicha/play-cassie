...
import com.clouidio.orm.api.z8spi.meta.DboColumnEmbedSimpleMeta;
...
import com.clouidio.orm.impl.meta.data.collections.SimpleAbstractCollection;
import com.clouidio.orm.impl.meta.data.collections.SimpleList;
...
public class MetaEmbeddedSimple<OWNER, T> extends MetaAbstractField<OWNER> {
	...
	// Converter converter, boolean isIndexed, boolean isPartitioned) {
	// metaDbo.setup(tableMeta, colName, field2.getType(), isIndexed,
	// isPartitioned);
	// super.setup(field2, colName);
	// this.converter = converter;
	// }
	public void setup(DboTableMeta t, Field field, String colName,
			Converter converter, Class<?> type) {
	...
	public String toString() {
		return "MetaEmbeddedSimple [field='"
				+ field.getDeclaringClass().getName() + "." + field.getName()
				+ "(field type=" + field.getType().getName() + "), columnName="
				+ columnName + "]";
	}
	...
	private Object translateFromColumnSet(Row row, OWNER entity,
		...
		List<byte[]> values = MetaToManyField.parseColNamePostfix(columnName,
				row);
		return null;
	}
	...
	private List<T> translateFromColumnList(Row row, OWNER entity,
			NoSqlSession session) {
		List<byte[]> values = MetaToManyField.parseColNamePostfix(columnName,
				row);
	...
	public void translateToColumn(InfoForIndex<OWNER> info) {
		...
		Collection<T> values = (Collection<T>) ReflectionUtil.fetchFieldValue(
				entity, field);
		if (values == null)
			values = new ArrayList<T>();
		Collection<T> toBeAdded = values; // all values in the list get added if
											// not an OurAbstractCollection
	...
	private void translateToColumnImpl(Collection<T> toBeAdded,
			RowToPersist row, Collection<T> toBeRemoved) {
		// removes first
		for (T p : toBeRemoved) {
