...
public class MetaProxyField<OWNER, PROXY> extends MetaAbstractField<OWNER> {
	...
	public String toString() {
		return "MetaProxyField [field='" + field.getDeclaringClass().getName()
				+ "." + field.getName() + "(field type="
				+ field.getType().getName() + "), columnName=" + columnName
				+ "]";
	}
	...
	public Object fetchField(Object entity) {
		...
		return value;
		// throw new
		// UnsupportedOperationException("only used for partitioning and multivalue column can't partition.  easy to implement if anyone else starts using this though, but for now unsupported");
	}
	...
	private StorageTypeEnum getStorageType() {
		StorageTypeEnum storageType = classMeta.getIdField().getMetaIdDbo()
				.getStorageType();
		return storageType;
	}
	...
	public byte[] translateValue(Object value) {
		...
		if (pk == null && value != null) {
			throw new ChildWithNoPkException(
					"You can't give us an entity with no pk!!!!  We use the pk to search the database index.  Please fix your bug");
		}
		return pk;
	}
	...
	public void setup(DboTableMeta tableMeta, Field field2, String colName,
			MetaAbstractClass<PROXY> classMeta, boolean isIndexed,
			boolean isPartitionedBy) {
