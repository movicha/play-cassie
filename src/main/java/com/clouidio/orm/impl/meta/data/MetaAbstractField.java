...
import com.clouidio.orm.api.base.Indexing;
...
public abstract class MetaAbstractField<OWNER> implements MetaField<OWNER> {
	...
	protected void removeIndexInfo(InfoForIndex<OWNER> info, Object value,
	...
	private void addIndexRemoves(InfoForIndex<OWNER> info, Object value,
			byte[] byteVal, StorageTypeEnum storageType) {
	...
	private void addToList(InfoForIndex<OWNER> info, byte[] oldIndexedVal,
			StorageTypeEnum storageType, byte[] pk, List<IndexData> indexList) {
	...
	protected void addIndexInfo(InfoForIndex<OWNER> info, Object value,
			byte[] byteVal, StorageTypeEnum storageType) {
	...
	private boolean isNeedPersist(OWNER entity, Object value,
			Map<Field, Object> fieldToValue) {
		if (!(entity instanceof NoSqlProxy))
			return true;
		else if (Indexing.isForcedIndexing())
			return true;
		else if (valuesEqual(fieldToValue, value)) // value is equal to previous
													// value
			return false;
	...
	private IndexData createAddIndexData(String columnFamily, byte[] byteVal,
			StorageTypeEnum storageType, byte[] pk, PartitionTypeInfo info) {
