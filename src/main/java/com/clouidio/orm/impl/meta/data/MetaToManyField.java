...
import com.clouidio.orm.impl.meta.data.collections.ListProxyFetchAll;
import com.clouidio.orm.impl.meta.data.collections.MapProxyFetchAll;
import com.clouidio.orm.impl.meta.data.collections.OurAbstractCollection;
import com.clouidio.orm.impl.meta.data.collections.SetProxyFetchAll;

public final class MetaToManyField<OWNER, PROXY> extends
	...
	public void translateFromColumn(Row row, OWNER entity, NoSqlSession session) {
		...
		if (field.getType().equals(Map.class))
			...
			throw new RuntimeException("bug, we do not support type="
					+ field.getType());
	...
	private Map translateFromColumnMap(Row row, OWNER entity,
		...
		// MapProxyFetchAll proxy = new MapProxyFetchAll(entity, session,
		// classMeta, keys, fieldForKey);
		return proxy;
	}
	...
	static List<byte[]> parseColNamePostfix(String columnName, Row row) {
		...
		// cannot have repeats right now. We could take the approach the column
		// name
		// would be <prefix><index><pk> such that duplicates are allowed and
		// when loaded
		// we would have to keep the index in the proxy so if removed, we could
		// put the col name
		// back together so we could remove it and add the new one. This is very
		// complex though when
		// it comes to removing one item and shifting all other column names by
		// one(ie. lots of removes/adds)
	...
	public void setup(DboTableMeta tableMeta, Field field, String colName,
			MetaAbstractClass<PROXY> classMeta, Field fieldForKey) {
	...
	public String toString() {
		return "MetaListField [field='" + field.getDeclaringClass().getName()
