...
public class MetaClassInheritance<T> extends MetaAbstractClass<T> {
	...
	/**
	 * For inheritance for a single table, we have multiple proxies for this
	 * class that we may need to create.
	 */
	...
	public MetaClassSingle<?> findOrCreate(Class<?> clazz, Class<?> parent) {
		NoSqlDiscriminatorColumn col = clazz
				.getAnnotation(NoSqlDiscriminatorColumn.class);
		if (col == null)
			throw new IllegalArgumentException(
					"Class "
							+ parent.getName()
							+ " in the NoSqlInheritance annotation, specifies a class"
							+ " that is missing the NoSqlDiscriminatorColumn annotation.  Class to add annotation to="
							+ clazz.getName());
		...
		// All the subclasses need to share the same meta Dbo object!!!! as it
		// is one table for the
	...
	private MetaClassSingle<T> retrieveMeta(Row row) {
		...
		String type = StandardConverters.convertFromBytes(String.class,
				column.getValue());
	...
	public Tuple<T> convertIdToProxy(Row row, NoSqlSession session,
		...
		if (row == null)
			throw new IllegalArgumentException(
					"bug, inheritance does not support who called this so you can't have inheritance along with it, we need to fix so let us know");
		MetaClassSingle<T> metaClassSingle = retrieveMeta(row);
		return metaClassSingle.convertIdToProxy(row, session, nonVirtKey,
				cacheLoadCallback);
	}
	...
	public List<MetaField<T>> getIndexedColumns() {
		throw new UnsupportedOperationException(
				"not done yet.  ie. don't have index support for inheritance tables yet");
	}
	...
	public boolean isPartitioned() {
		throw new UnsupportedOperationException(
				"not done yet.  ie. don't have partition support for inheritance tables yet");
	}
