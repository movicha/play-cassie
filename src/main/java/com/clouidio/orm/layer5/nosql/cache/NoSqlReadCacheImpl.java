...
public class NoSqlReadCacheImpl implements NoSqlSession, Cache {
	...
	private static final Logger log = LoggerFactory
			.getLogger(NoSqlReadCacheImpl.class);
	...
	public void persistIndex(DboTableMeta colFamily, String indexColFamily,
			byte[] rowKey, IndexColumn columns) {
	...
	public AbstractCursor<KeyValue<Row>> find(DboTableMeta colFamily,
		...
		// NOTE: I would put a finally to clear out the threadlocal normally BUT
		// sometimes log statements may
		// cause further finds to be called which come in here as well and on
		// their way BACK up the stack, they set
		// the cache to NULL before the find actually happens(ie. very bad).
		CacheThreadLocal.setCache(c);

		// A layer below will read the thread local and pass it to lowest layer
		// to use
		AbstractCursor<KeyValue<Row>> rowsFromDb = session.find(colFamily,
				rowKeys, skipCache, cacheResults, batchSize);
		return rowsFromDb;
	}
	...
	public RowHolder<Row> fromCache(DboTableMeta colFamily, byte[] key) {
		if (key == null)
			throw new IllegalArgumentException(
					"CF="
							+ colFamily
							+ " key is null and shouldn't be....(this should be trapped in higher level exception telling us which index is corrupt");
	...
	public void cacheRow(DboTableMeta colFamily, byte[] key, Row r) {
		// NOTE: We cache null rows as on a user.getYYYEntites(), the loaded
		// entities may be null though the user
		// get a List<YYYEntity> and all are there but he can check if they are
		// really there with
		// mgr.checkRowExists(entity) and that will just hit the cache
		TheKey k = new TheKey(colFamily.getColumnFamily(), key);
		RowHolder<Row> holder = new RowHolder<Row>(key, r); // r may be null so
															// we are caching
															// null here
		cache.put(k, holder);
	}
