...
import com.clouidio.orm.api.z8spi.action.RemoveEnum;
...
public class NoSqlWriteCacheImpl implements NoSqlSession {
	...
	@Override
	public void persistIndex(DboTableMeta cf, String indexColFamily,
	...
	@Override
	public void removeFromIndex(DboTableMeta cf, String indexColFamily,
	...
	public AbstractCursor<KeyValue<Row>> find(DboTableMeta colFamily,
			DirectCursor<byte[]> rowKeys, boolean skipCache,
			boolean cacheResults, Integer batchSize) {
		int size = 500;
		if (batchSize != null)
			size = batchSize;
		return rawSession
				.find(colFamily, rowKeys, null, size, null, ormSession);
	}

	private List<Row> find(DboTableMeta colFamily, List<byte[]> keys,
			boolean cacheResults) {
		AbstractCursor<KeyValue<Row>> results = find(colFamily,
				new IterableWrappingCursor<byte[]>(keys), false, cacheResults,
				null);
	...
	public void flush() {
		...
		// special case here...if any persists were of the DboTableMeta, we
		// should create table now
		for (byte[] key : newTables) {
			String colFamily = StandardConverters.convertFromBytes(
					String.class, key);
	...
	public AbstractCursor<IndexColumn> scanIndex(ScanInfo info, Key from,
		...
		return rawSession
				.scanIndex(info, from, to, batchSize, null, ormSession);
	}
