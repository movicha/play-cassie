...
public class CursorForPrimaryKey implements DirectCursor<IndexColumnInfo> {
	...
	public CursorForPrimaryKey(ViewInfo viewInfo, DboColumnMeta info,
	...
	public String toString() {
		...
		String retVal = "CursorForPrimaryKey[" + tabs + cursor + tabs
				+ viewInfo + tabs + "]";
	...
	private Holder<IndexColumnInfo> buildHolder(Holder<KeyValue<Row>> holder) {
		...
		if (pk != null) {
			c.setPrimaryKey(colMeta.getOwner().getIdColumnMeta()
					.unformVirtRowKey(pk));
			c.setIndexedValue(colMeta.getOwner().getIdColumnMeta()
					.unformVirtRowKey(pk));
			c.setColumnName(colMeta.getOwner().getIdColumnMeta()
					.getColumnName());
			c.setValue(pk);
		}
