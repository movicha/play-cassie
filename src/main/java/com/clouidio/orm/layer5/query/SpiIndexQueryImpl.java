package com.clouidio.orm.layer5.query;
...
import com.clouidio.orm.api.z5api.IndexColumnInfo;
import com.clouidio.orm.api.z5api.NoSqlSession;
import com.clouidio.orm.api.z5api.SpiQueryAdapter;
...
import com.clouidio.orm.api.z8spi.iter.IterableWrappingCursor;
...
import com.clouidio.orm.parser.antlr.ChildSide;
import com.clouidio.orm.parser.antlr.ExpressionNode;
import com.clouidio.orm.parser.antlr.JoinInfo;
import com.clouidio.orm.parser.antlr.JoinMeta;
import com.clouidio.orm.parser.antlr.JoinType;
import com.clouidio.orm.parser.antlr.NoSqlLexer;
import com.clouidio.orm.parser.antlr.ParsedNode;
import com.clouidio.orm.parser.antlr.PartitionMeta;
import com.clouidio.orm.parser.antlr.StateAttribute;
import com.clouidio.orm.parser.antlr.ViewInfoImpl;
...
public class SpiIndexQueryImpl implements SpiQueryAdapter {
	...
	private ByteArray getParameter(String parameterName) {
		...
		if (result == null)
			throw new IllegalStateException(
					"You did not call query.setParameter(\"" + parameterName
							+ "\", <yourvalue>) and that parameter is required");
		return result;
	}

	@Override
	public DirectCursor<IndexColumnInfo> getResultList(
			Set<ViewInfo> alreadyJoinedViews, String indexedColumn) {
		if (alreadyJoinedViews == null || alreadyJoinedViews.size() != 0)
			throw new IllegalArgumentException(
					"You must pass us a non-null Set that is EMPTY and not null");
		DirectCursor<IndexColumnInfo> cursor = getResultListImpl(
				alreadyJoinedViews, indexedColumn);
		return cursor;
	}

	public DirectCursor<IndexColumnInfo> getResultListImpl(
			Set<ViewInfo> alreadyJoinedViews, String indexedColumn) {
		ExpressionNode root = spiMeta.getASTTree();
		if (root == null) {
			ViewInfoImpl tableInfo = (ViewInfoImpl) spiMeta.getTargetViews()
					.get(0);
			...
			AbstractCursor<IndexColumn> scan = session.scanIndex(scanInfo,
					null, null, batchSize);
	...
	private ScanInfo createScanInfo(ViewInfoImpl tableInfo,
			DboColumnMeta metaCol) {
		PartitionMeta partitionMeta = tableInfo.getPartition();
		...
		if (partitionMeta != null) {
			...
			byte[] partId = retrieveValue(colMeta,
					(ExpressionNode) partitionMeta.getNode());
			Object partIdObj = colMeta.convertFromStorage2(partId);
			partitionId = colMeta.convertTypeToString(partIdObj);
		}

		ScanInfo scanInfo = ScanInfo.createScanInfo(metaCol, partitionBy,
				partitionId);
		return scanInfo;
	}

	private DirectCursor<IndexColumnInfo> processExpressionTree(
			ExpressionNode parent, Set<ViewInfo> alreadyJoinedViews) {
		...
		default:
			throw new UnsupportedOperationException("bug, unsupported type="
					+ type);
		}
	}

	private DirectCursor<IndexColumnInfo> processAndOr(ExpressionNode root,
			Set<ViewInfo> alreadyJoinedViews) {
		...
		DirectCursor<IndexColumnInfo> leftResults = processExpressionTree(left,
				alreadyJoinedViews);
		DirectCursor<IndexColumnInfo> rightResults = processExpressionTree(
		...
		if (joinType == JoinType.INNER || joinType == JoinType.LEFT_OUTER) {
			// We need to proxy the right results to translate to the same
			// primary key as the
			// left results and our And and Or Cursor can then take care of the
			// rest
			...
			// FROM an ORM perspective, we join to smaller tables in general as
			// we don't want to blow out memory so do the
			// join first(ie. we process left sides first in and and or cursors)
			CursorForJoin temp = new CursorForJoin(newView, leftView,
					leftResults, joinType);
			temp.setColMeta(col);
		...
		if (root.getType() == NoSqlLexer.AND) {
			CursorForAnd cursor = new CursorForAnd(leftView, leftResults,
					rightView, rightResults);
			// AND always returns LESS results(or same) than the left or right
			// sides,
			// sooooo, we cache results if there is less than 500 results
			return new CachingCursor<IndexColumnInfo>(cursor);
		} else {
			// Since OR always returns MORE results(or the same) as the left or
			// right views
			// There is no need to use a caching cursor as the people below us
			// have a caching
			// cursor AND there would be no performance benefit
			return new CursorForOr(leftView, leftResults, rightView,
					rightResults);
		}
	}

	private DirectCursor<IndexColumnInfo> processRangeExpression(
			ExpressionNode root, Set<ViewInfo> alreadyJoinedViews) {
		StateAttribute attr;
		if (root.getType() == NoSqlLexer.BETWEEN) {
			ExpressionNode grandChild = root.getChild(ChildSide.LEFT).getChild(
					ChildSide.LEFT);
		...
		if (info.isIndexed()) {
		...
		} else if (info.getOwner().getIdColumnMeta().getColumnName()
				.equals(info.getColumnName())) {
			...
			throw new IllegalArgumentException(
					"You cannot have '"
							+ info.getColumnName()
							+ "' in your sql query since "
							+ info.getColumnName()
							+ " is neither a Primary Key nor a column with @Index annotation on the field in the entity");
	}

	private DirectCursor<IndexColumnInfo> processIndexColumn(
			ExpressionNode root, ScanInfo scanInfo, ViewInfoImpl viewInfo,
			DboColumnMeta info) {
		...
		if (root.getType() == NoSqlLexer.EQ) {
			...
			for (ParsedNode keyNode : keys) {
				...
				byte[] virtualkey = info.getOwner().getIdColumnMeta()
						.formVirtRowKey(data);
			...
			throw new UnsupportedOperationException("not supported yet. type="
					+ root.getType());
		DirectCursor<IndexColumnInfo> processKeys = processKeys(viewInfo, info,
				scan);
		return processKeys;
	}

	@SuppressWarnings("unchecked")
	private DirectCursor<IndexColumnInfo> processPrimaryKey(
			ExpressionNode root, ScanInfo scanInfo, ViewInfoImpl viewInfo,
			DboColumnMeta info) {
		...
		if (root.getType() == NoSqlLexer.EQ) {
			byte[] data = retrieveValue(info, root.getChild(ChildSide.RIGHT));
			if (data == null)
				throw new UnsupportedOperationException("Primary key "
						+ info.getColumnName() + " cannot be null");
			byte[] virtualkey = info.getOwner().getIdColumnMeta()
					.formVirtRowKey(data);
			List<byte[]> keyList = new ArrayList<byte[]>();
			keyList.add(virtualkey);
			scan = session.find(info.getOwner(),
					new IterableWrappingCursor<byte[]>(keyList), false, true,
					batchSize);
		} else if (root.getType() == NoSqlLexer.IN) {
			List<byte[]> keyList = new ArrayList<byte[]>();
			List<ParsedNode> keys = root.getChildrenForIn();
			...
			throw new UnsupportedOperationException(
					"Other operations not supported yet for Primary Key. Use @NoSQLIndexed for Primary Key.type="
							+ root.getType());
		DirectCursor<IndexColumnInfo> processKeys = processKeysforPK(viewInfo,
				info, scan);
		return processKeys;
	}
	...
	private Key createRightKey(ExpressionNode node, DboColumnMeta info) {
		...
		if (node.getType() == NoSqlLexer.LT)
			...
			throw new RuntimeException(
					"bug, should never happen, but should be easy to fix this one");
	}
	...
	private Key createLeftKey(ExpressionNode node, DboColumnMeta info) {
		...
		if (node.getType() == NoSqlLexer.GT)
			...
			throw new RuntimeException(
					"bug, should never happen, but should be easy to fix this one. type="
							+ node.getType());
	}

	private DirectCursor<IndexColumnInfo> processKeys(ViewInfo viewInfo,
			DboColumnMeta info, AbstractCursor<IndexColumn> scan) {
		DirectCursor<IndexColumnInfo> cursor = new CursorSimpleTranslator(
				viewInfo, info, scan);
		return new CachingCursor<IndexColumnInfo>(cursor);
	}

	private DirectCursor<IndexColumnInfo> processKeysforPK(ViewInfo viewInfo,
			DboColumnMeta info, AbstractCursor<KeyValue<Row>> scan) {
		DirectCursor<IndexColumnInfo> cursor = new CursorForPrimaryKey(
				viewInfo, info, scan);
		return new CachingCursor<IndexColumnInfo>(cursor);
	}
	...
	private byte[] processConstant(DboColumnMeta info, ExpressionNode node) {
		...
		if (info.isJodaType()) {
			...
			if (info.getClassType().getName().equals("org.joda.time.DateTime")) {
			...
			} else if (info.getClassType().getName()
					.equals("org.joda.time.LocalDateTime")) {
				fmt = ISODateTimeFormat.dateTime();
				LocalDateTime localDateTime = fmt.parseLocalDateTime(constant
						.toString());
				return StandardConverters.convertToBytes(localDateTime);
			} else if (info.getClassType().getName()
					.equals("org.joda.time.LocalDate")) {
				...
				return StandardConverters.convertToBytes(localDate);
					...
					.equals("org.joda.time.LocalTime")) {
	...
	public void setBatchSize(int batchSize) {
		if (batchSize <= 0)
			throw new IllegalArgumentException(
					"batchSize must be 1 or greater, but really, please don't use 1, use something like 500(the default anyways)");
