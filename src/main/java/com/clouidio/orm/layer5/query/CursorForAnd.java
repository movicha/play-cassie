...
public class CursorForAnd implements DirectCursor<IndexColumnInfo> {
	...
	public CursorForAnd(ViewInfo leftView2,
			DirectCursor<IndexColumnInfo> leftResults, ViewInfo rightView2,
			DirectCursor<IndexColumnInfo> rightResults) {
	...
	public String toString() {
		...
		String retVal = "CursorForAnd[" + tabs + leftResults + "," + tabs
				+ rightResults + tabs + "]";
	...
	public Holder<IndexColumnInfo> nextImpl() {
		while (true) {
			...
			if (cachedMap != null) {
				...
				// Need to change to lookahead joining here as well as switch
				// join types on the fly
				// This stinks as we have to re-read from the database every
				// 1000 rows!!!! We should find out if
				// we can do any kind of counting or something so we don't have
				// to go back to database every time
				rightResults.beforeFirst();
	...
	private Holder<IndexColumnInfo> runFullInnerLoopOnceBackward(
			IndexColumnInfo previous) {
		Map<ByteArray, IndexColumnInfo> pkToRightSide = new HashMap<ByteArray, IndexColumnInfo>();
		Holder<IndexColumnInfo> matchedResult = null;
		while (true) {
			Holder<IndexColumnInfo> previousFromCursor = rightResults
					.previousImpl();
	...
	private Holder<IndexColumnInfo> runInnerLoopBackward(
			IndexColumnInfo previous) {
		while (true) {
			Holder<IndexColumnInfo> prevFromCursor = rightResults
					.previousImpl();
			if (prevFromCursor == null)
				break;
