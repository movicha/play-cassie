...
public class CursorForJoin extends AbstractCursor<IndexColumnInfo> {
	...
	public CursorForJoin(ViewInfo view, ViewInfo rightView,
			DirectCursor<IndexColumnInfo> rightResults, JoinType joinType) {
	...
	public String toString() {
		...
		String retVal = "CursorForJoin[" + tabs + "rightCursor=" + rightResults
				+ tabs + "rightView=" + rightView + tabs
				+ "cachedCursorNewView=" + cachedFromNewView + tabs
				+ "newView=" + newView + tabs + "cachedFRomRight="
				+ cachedFromRightResults + tabs + "]";
	...
	private Holder<IndexColumnInfo> fetchAnotherPrevious() {
		...
		cachedFromRightResults = rightListResults.listIterator();
		while (cachedFromRightResults.hasNext())
			cachedFromRightResults.next();
		cachedFromNewView = session.scanIndex(scanInfo, values);
	...
	private Holder<IndexColumnInfo> createResult(
			com.clouidio.orm.api.z8spi.iter.AbstractCursor.Holder<IndexColumn> next) {
		IndexColumn indCol = next.getValue();
		IndexColumnInfo info = new IndexColumnInfo();

		if (cachedFromRightResults.hasNext()) {
			// We need to compare lastCachedRightSide with our incoming to make
			// see if they pair up
			// or else move to the next right side answer.
			ByteArray pkOfRightView = lastCachedRightSide
					.getPrimaryKey(rightView);
