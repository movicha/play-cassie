...
public class CursorForOr implements DirectCursor<IndexColumnInfo> {
	...
	public CursorForOr(ViewInfo leftView,
			DirectCursor<IndexColumnInfo> leftResults2, ViewInfo rightView,
			DirectCursor<IndexColumnInfo> rightResults2) {
	...
	private Holder<IndexColumnInfo> nextImpl(boolean reverse) {
		if (currLeftVal != null && currRightVal != null) {
			int compareResult = currLeftVal
					.getValue()
					.getPrimaryKey(leftView)
					.compareTo(currRightVal.getValue().getPrimaryKey(rightView));
			if (reverse)
			...
			else { // they are the same, use either, and skip the unused one
