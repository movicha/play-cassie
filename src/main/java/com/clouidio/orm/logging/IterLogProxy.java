...
public class IterLogProxy implements Iterable<byte[]> {
	...
	public Iterator<byte[]> iterator() {
		return new IteratorLogProxy(prefix, databaseInfo, colFamily,
				rowKeys.iterator());
	}
	...
	private static class IteratorLogProxy implements Iterator<byte[]> {
		...
		public boolean hasNext() {
			...
			if (log.isInfoEnabled() && !hasNext && !alreadyLogged
					&& realKeys.size() > 0) {
				// we are finished and can now log the keys about to be looked
				// up
				log.info(prefix + " CF=" + colFamily + " finding keys="
						+ realKeys);
				alreadyLogged = true;
			}
			return hasNext;
		...
		public byte[] next() {
			...
			if (meta != null && log.isInfoEnabled()) {
				try {
					...
					String str = meta.getIdColumnMeta()
							.convertTypeToString(obj);
					realKeys.add(str);
				} catch (Exception e) {
