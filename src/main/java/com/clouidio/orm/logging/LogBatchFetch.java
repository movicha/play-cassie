...
public class LogBatchFetch implements BatchListener {
	...
	private void logInfo(int numFetched, String bSize, long total) {
		switch (findType) {
		case FIND:
			log.info("[rawlogger]" + cfAndIndex + " Find took=" + total
					+ " ms for batchSize=" + bSize + " numFetched="
					+ numFetched);
			break;
		case COLUMN_SLICE:
			log.info("[rawlogger]" + cfAndIndex + " Column slice took=" + total
					+ " ms for batchSize=" + bSize + " numFetched="
					+ numFetched);
			break;
		case RANGE_SLICE:
			log.info("[rawlogger]" + cfAndIndex + " Index slice took=" + total
					+ " ms for batchSize=" + bSize + " numFetched="
					+ numFetched);
			break;
		case NON_CONTIGUOUS:
			log.info("[rawlogger]" + cfAndIndex
					+ " Non-contiguous columns fetch took=" + total
					+ " ms for batchSize=" + bSize + " numFetched="
					+ numFetched);
			break;
		default:
