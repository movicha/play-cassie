...
import com.clouidio.orm.api.z8spi.CacheThreadLocal;
...
public class NoSqlRawLogger implements NoSqlRawSession {
	...
	private static final Logger log = LoggerFactory
			.getLogger(NoSqlRawLogger.class);
	@Inject
	@Named("main")
	...
	public void sendChanges(List<Action> actions, MetaLookup ormFromAbove) {
		...
		if (log.isInfoEnabled()) {
			...
			log.info("[rawlogger] Sending Changes to server took(including spi plugin)="
					+ total + " ms");
		}
	}
	...
	private String convert(RemoveIndex r) {
		...
		msg += "[rowkey="
				+ StandardConverters.convertFromBytesNoExc(String.class,
						r.getRowKey()) + "]";
	...
	public AbstractCursor<Column> columnSlice(DboTableMeta colFamily,
		...
		if (log.isInfoEnabled()) {
			log.info("[rawlogger] CF="
					+ colFamily
					+ " column slice(we have not meta info for column Slices, use scanIndex maybe?)");
			list = new LogBatchFetch("basic column slice", l, batchSize,
					ScanType.COLUMN_SLICE);
		}

		AbstractCursor<Column> ret = session.columnSlice(colFamily, rowKey,
				from, to, batchSize, list, mgr);
	...
	public AbstractCursor<IndexColumn> scanIndex(ScanInfo scanInfo,
			List<byte[]> values, BatchListener l, MetaLookup mgr) {
		BatchListener list = l;
		if (log.isInfoEnabled()) {
			String cfAndIndex = logColScan2(scanInfo, values);
			list = new LogBatchFetch(cfAndIndex, l, null,
					ScanType.NON_CONTIGUOUS);
		}

		AbstractCursor<IndexColumn> cursor = session.scanIndex(scanInfo,
				values, list, mgr);
		return cursor;
	}
	...
	private String logColScan2(ScanInfo info, List<byte[]> values) {
		try {
			...
			log.info("[rawlogger] (Exception trying to log column scan on index cf="
					+ info.getIndexColFamily()
					+ " for cf="
					+ info.getEntityColFamily());
			return info.getIndexColFamily() + " " + info.getEntityColFamily();
		}
	}

	private String logColScan2Impl(ScanInfo info, List<byte[]> values) {
		String rowKey = StandardConverters.convertFromBytesNoExc(String.class,
				info.getRowKey());
		String cfAndIndex = "CF=" + info.getEntityColFamily() + " index="
				+ rowKey;
		...
		msg += " finding non-contiguous keys index rowkey=" + rowKey
				+ " for keys:" + strVals;
	...
	private String logColScanImpl(ScanInfo info, Key from, Key to,
		...
		msg += " scanning index for value in range:" + range
	...
	public void clearDatabase() {
		...
		if (log.isInfoEnabled()) {
			long total = System.currentTimeMillis() - time;
			log.info("[rawlogger] clearDatabase took(including spi plugin)="
					+ total + " ms");
		}
	}
	...
	public AbstractCursor<KeyValue<Row>> find(DboTableMeta colFamily,
			DirectCursor<byte[]> rowKeys, Cache realCache, int batchSize,
			BatchListener l, MetaLookup mgr) {
		BatchListener list = l;
		// NOTE: get the cache here before the below log statement which used to
		// clear out the trhead local as it ends
		// up doing a find through the cache layer that used to clear the thread
		// local
		...
		if (cache == null)
			throw new IllegalArgumentException(
					"bug, above layer needs to call CacheThradLocal.setCache(cache) with non-null cache");
	...
	// Iterable<byte[]> rKeys) {
	// //Astyanax will iterate over our iterable twice!!!! so instead we will
	// iterate ONCE so translation
	// //only happens ONCE and then feed that to the SPI(any other spis then who
	// iterate twice are ok as well then)
	//
	...
	// //This is where the cursor is read from causing queries to hit the
	// database
	...
	// long total = System.currentTimeMillis() - time;
	// if(allKeys.size() > 0) //we really only did a find if there were actual
	// keys passed in
	...
	// ret = session.find(colFamily, allKeys);
	//
	// //UNFORTUNATELY, astyanax's result is NOT ORDERED by the keys we provided
	// so, we need to iterate over the whole thing here
	// //into our own List :( :( . OTHER SPI's may not be ORDERED EITHER so we
	// iterate here for all of them.
	// List<KeyValue<Row>> results = new ArrayList<KeyValue<Row>>();
	// Map<ByteArray, KeyValue<Row>> map = new HashMap<ByteArray,
	// KeyValue<Row>>();
	// while(true) {
	...
	//
	//
	// ProxyTempCursor<KeyValue<Row>> proxy = new
	// ProxyTempCursor<KeyValue<Row>>(results);
	// return proxy;
	// }
