...
import com.clouidio.orm.api.exc.StorageMissingEntitesException;
import com.clouidio.orm.api.exc.TooManyResultException;
import com.clouidio.orm.api.exc.TypeMismatchException;
...
import com.clouidio.orm.layer3.typed.IterableProxy;
...
public class QueryAdapter<T> implements Query<T> {
	...
	public void setup(MetaClass<T> target, SpiMetaQuery metaQuery,
			SpiQueryAdapter indexQuery, BaseEntityManagerImpl entityMgr,
			Class clazz) {
	...
	public Query<T> setParameter(String name, Object value) {
		...
		if (typeInfo == null) {
			...
			throw new UnsupportedOperationException(
					"not done here yet, need to validate constant type");
		...
		MetaField metaField = metaClass.getMetaFieldByCol(targetSubclass,
				columnName);

		Field field = metaField.getField();
		Class fieldType = field.getType();
		// Are actual type will never be a primitive because of autoboxing. When
		// the param
		// is passed in, it becomes an Long, Integer, etc. so we need to convert
		// here
		...
		if (value != null) {
			...
			if (!objectFieldType.isAssignableFrom(actualType)) {
				throw new TypeMismatchException("value [" + value
						+ "] is not the correct type for the parameter='"
						+ name
						+ "' from inspecting the Entity.  Type should be=["
						+ fieldType + "]");
			}
		}
	...
	public T getSingleObject() {
		...
		if (results.next())
			throw new TooManyResultException(
					"Too many results to call getSingleObject...call getResultList instead");
		return kv.getValue();
	}
	...
	private Cursor<KeyValue<T>> getResults(boolean cacheResults,
		...
		// BIG NOTE: Here, we could return all the keys from the join so we can
		// eagerly fetch other entities as well
		// instead of waiting for the user to loop through those entities AND if
		// user accesses those entites, we could block
		// while the load is happening too in the background
		String query = meta.getQuery();
		AbstractCursor<KeyValue<T>> results = mgr.findAllImpl2(mainMetaClass,
				mainView, indice, query, cacheResults, batchSize);
	...
	public List<T> getResultList(int firstResult, Integer maxResults) {
		// IF batch size was not set, let's set it to max results here for the
		// case where firstResult=0,
		// this will make it very very fast. For a case of maxResults=2 and
		// firstResult=500, this will be slow sooooo
		// make sure you set the batchSize yourself if you want maxResults=<low
		// number>
		...
		try {
			...
			log.trace("converting row not found into stored entities missing",
					e);
			throw new StorageMissingEntitesException(
					foundElements,
					"Your index refers to rows that no longer exist in the nosql store",
					e);
		}
	}

	private List<T> getEntities(AbstractCursor<KeyValue<T>> all,
			List<T> foundElements, int firstResult, Integer maxResults) {
		...
		while (true) {
			...
			if (counter < firstResult) {
				...
				continue; // skip it
					...
					&& counter >= firstResult + maxResults)
		...
		if (exc != null) {
			...
			throw new StorageMissingEntitesException(
					foundElements,
					"Your index refers to rows that no longer exist in the nosql store(use getResults method instead to avoid this exception until you call getValue on actual element)",
					exc);
		}
		return entities;
	}
