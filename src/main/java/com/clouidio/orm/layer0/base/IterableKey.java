...
public class IterableKey<T> implements Iterable<byte[]> {
	...
	public String toString() {
		...
		String retVal = "IterableKey(getNextKey)[" + tabs + "idMeta=" + meta
				+ tabs + "iterable=" + keys + tabs + "]";
	...
	private static class IteratorProxy<T> implements Iterator<byte[]> {
		...
		public String toString() {
			String tabs = StringLocal.getAndAdd();
			String retVal = "IteratorProxy(getNextKey)[" + tabs + "idMeta="
					+ idMeta + tabs + "iterator=" + iterator + tabs + "]";
			StringLocal.set(tabs.length());
			return retVal;
		}
		...
		public byte[] next() {
			...
			if (nonVirtKey == null)
				throw new IllegalArgumentException(
						"You supplied a null key to your list when calling findAll method.  We can't lookup null as a key");
			// NOTE: Next iterator CONVERTS to virtual key so do not do it
			// here!!!!
			return nonVirtKey;
		}
