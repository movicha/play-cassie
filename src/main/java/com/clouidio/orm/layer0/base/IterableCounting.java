...
public class IterableCounting implements Iterable<byte[]> {
	...
	public IterableCounting(AbstractIterator<byte[]> keysIterator,
			int batchSize2) {
	...
	public Iterator<byte[]> iterator() {
		if (alreadyRun)
			throw new IllegalStateException(
					"BUG, This iterable can only be run once as it is proxy to an Iterator that CAN ONLY be run once");
		alreadyRun = true;
	...
	private static class CountingIterator extends AbstractIterator<byte[]> {
		...
		public CountingIterator(AbstractIterator<byte[]> keysIterator,
				int batchSize2) {
			this.keysIterator = keysIterator;
			this.batchSize = batchSize2;
		}
