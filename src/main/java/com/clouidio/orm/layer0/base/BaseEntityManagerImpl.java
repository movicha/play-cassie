...
import com.clouidio.orm.api.z8spi.MetaLoader;
...
import com.clouidio.orm.api.z8spi.iter.IterToVirtual;
...
import com.clouidio.orm.impl.meta.data.MetaClass;
import com.clouidio.orm.impl.meta.data.MetaIdField;
import com.clouidio.orm.impl.meta.data.MetaInfo;
import com.clouidio.orm.impl.meta.data.NoSqlProxy;
import com.clouidio.orm.layer3.typed.IndiceCursorProxy;
import com.clouidio.orm.layer3.typed.NoSqlTypedSessionImpl;
...
public class BaseEntityManagerImpl implements NoSqlEntityManager, MetaLookup,
		...
		if (metaClass == null)
			throw new IllegalArgumentException(
					"Entity type="
							...
							+ " was not scanned and added to meta information on startup.  It is either missing @NoSqlEntity annotation or it was not in list of scanned packages");
	...
	public void put(Object entity) {
		...
		if (idField.isAutoGen() && id != null
				&& !(entity instanceof NoSqlProxy)) {
			// We do NOT have the data from the database IF this is NOT a
			// NoSqlProxy and we need it since this is an
			// update
			needRead = true;
		}
		putImpl(entity, needRead, metaClass);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void putImpl(Object originalEntity, boolean needRead,
			MetaClass metaClass) {
		...
		if (metaDbo.isEmbeddable())
			throw new IllegalArgumentException(
					"Entity type="
							+ entity.getClass().getName()
							+ " can not be saved as it is Embedded Entity. And Embeddable entitites are only saved along with their parent entity");
		// This is if we need to be removing columns from the row that
		// represents the entity in a oneToMany or ManyToMany
		// as the entity.accounts may have removed one of the accounts!!!
		if (row.hasRemoves())
			session.remove(metaDbo, row.getKey(), row.getColumnNamesToRemove());

		// NOW for index removals if any indexed values change of the entity, we
		// remove from the index
		for (IndexData ind : row.getIndexToRemove()) {
			session.removeFromIndex(metaDbo, ind.getColumnFamilyName(),
					ind.getRowKeyBytes(), ind.getIndexColumn());
		}

		// NOW for index adds, if it is a new entity or if values change, we
		// persist those values
		for (IndexData ind : row.getIndexToAdd()) {
			session.persistIndex(metaDbo, ind.getColumnFamilyName(),
					ind.getRowKeyBytes(), ind.getIndexColumn());
		}
	...
	public <T> T find(Class<T> entityType, Object key) {
		if (key == null)
			throw new IllegalArgumentException(
					"key must be supplied but was null");
	...
	public <T> Cursor<KeyValue<T>> findAll(Class<T> entityType,
		...
		if (meta == null)
			throw new IllegalArgumentException(
					"Class type="
							+ entityType.getName()
							+ " was not found, please check that you scanned the right package and look at the logs to see if this class was scanned");

		Iterable<byte[]> iter = new IterableKey<T>(meta, keys);
		Iterable<byte[]> virtKeys = new IterToVirtual(meta.getMetaDbo(), iter);

		// we pass in null for batch size such that we do infinite size or
		// basically all keys passed into this method in one
		// shot
		return findAllImpl2(meta, new IterableWrappingCursor<byte[]>(virtKeys),
				null, true, null);
	}

	<T> AbstractCursor<KeyValue<T>> findAllImpl2(MetaClass<T> meta,
			ViewInfo mainView, DirectCursor<IndexColumnInfo> keys,
			String query, boolean cacheResults, Integer batchSize) {
		// OKAY, so this gets interesting. The noSqlKeys could be a proxy
		// iterable to
		// millions of keys with some batch size. We canNOT do a find inline
		// here but must do the find in
		// batches as well
		IndiceCursorProxy indiceCursor = new IndiceCursorProxy(mainView, keys);
		DirectCursor<byte[]> virtKeys = new IndiceToVirtual(meta.getMetaDbo(),
				indiceCursor);
		return findAllImpl2(meta, virtKeys, query, cacheResults, batchSize);
	}

	<T> AbstractCursor<KeyValue<T>> findAllImpl2(MetaClass<T> meta,
			DirectCursor<byte[]> keys, String query, boolean cacheResults,
			Integer batchSize) {
		boolean skipCache = query != null;
		AbstractCursor<KeyValue<Row>> cursor = session.find(meta.getMetaDbo(),
				keys, skipCache, cacheResults, batchSize);
	...
	public <T> Query<T> createNamedQuery(Class<T> forEntity, String namedQuery) {
		...
		SpiQueryAdapter spiAdapter = metaQuery
				.createQueryInstanceFromQuery(session);

		// We cannot return MetaQuery since it is used by all QueryAdapters and
		// each QueryAdapter
		// runs in a different thread potentially while MetaQuery is one used by
		// all threads
	...
	void saveMetaData() {
		...
		// DboDatabaseMeta existing = tempMgr.find(DboDatabaseMeta.class,
		// DboDatabaseMeta.META_DB_ROWKEY);
		// if(existing != null)
		// throw new
		// IllegalStateException("Your property NoSqlEntityManagerFactory.AUTO_CREATE_KEY is set to 'create' which only creates meta data if none exist already but meta already exists");
		...
		// NOW, on top of the ORM entites, we have 3 special index column
		// families of String, BigInteger and BigDecimal
		// which are one of the types in the composite column name.(the row keys
		// are all strings). The column names
		// are <value being indexed of String or BigInteger or
		// BigDecimal><primarykey><length of first value> so we can
		// sort it BUT we can determine the length of first value so we can get
		// to primary key.

		for (StorageTypeEnum type : StorageTypeEnum.values()) {
			// Do we want a byte[] index that could be used with == but not <
			// and not >, etc. etc.
	...
	public void remove(Object entity) {
		...
		if (!metaClass.hasIndexedField(entity)) {
			...
			// then we don't have the database information for indexes so we
			// need to read from the database
			proxy = find(metaClass.getMetaClass(), pk);
		}

		List<IndexData> indexToRemove = metaClass.findIndexRemoves(
				(NoSqlProxy) proxy, rowKey);

		// REMOVE EVERYTHING HERE, we are probably removing extra and could
		// optimize this later
