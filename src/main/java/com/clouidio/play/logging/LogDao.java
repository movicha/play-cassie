package com.clouidio.play.logging;
...
import com.clouidio.orm.api.base.spi.UniqueKeyGenerator;
...
public class LogDao {
	...
	public static Cursor<LogEvent> fetchAllLogs(NoSqlEntityManager mgr,
			int batchSize) {
		ServersThatLog servers = NoSql.em().find(ServersThatLog.class,
				ServersThatLog.THE_ONE_KEY);
	...
	public static Cursor<KeyValue<LogEvent>> fetchLatestLogs(
			NoSqlEntityManager mgr, int count) {
		ServersThatLog servers = NoSql.em().find(ServersThatLog.class,
				ServersThatLog.THE_ONE_KEY);
	...
	private static List<String> createKeys(List<String> serverNames, int min,
			int max) {
	...
	private static void findLatestRow(NoSqlEntityManager mgr,
			String serverName, Answer ans) {
		int row = ans.getRow();
		List<String> keys = new ArrayList<String>();
	...
	private static Iterable<KeyValue<LogEvent>> fetchSessionLogs(
			NoSqlEntityManager mgr, String sessionId, int numDigits) {
		return LogEvent.findBySession(mgr, sessionId, numDigits);
	}

	public static List<LogEvent> fetchOrderedSessionLogs(
			NoSqlEntityManager mgr, String sessionId, int numDigits) {
		Iterable<KeyValue<LogEvent>> iter = fetchSessionLogs(mgr, sessionId,
				numDigits);
	...
	private static class LogIterator extends AbstractCursor<LogEvent> {
		...
		// TODO:JSC none of this works if I can't figure out what the last
		// index/size/number of results is...
		@Override
		...
		@Override
		public com.clouidio.orm.api.z8spi.iter.AbstractCursor.Holder<LogEvent> nextImpl() {
			if (event == null)
				fetchNewBatch();
		...
		@Override
		public com.clouidio.orm.api.z8spi.iter.AbstractCursor.Holder<LogEvent> previousImpl() {
			if (event == null)
				fetchNewBatchFromEnd();
		...
		private void fetchNewBatch() {
			List<String> keys = createKeys(serverNames, counter, counter
					+ batchSize);
			counter = counter + batchSize;
			mgr.clear();

			event = (AbstractCursor<KeyValue<LogEvent>>) mgr.findAll(
					LogEvent.class, keys);
		}

		private void fetchNewBatchFromEnd() {
			List<String> keys = createKeys(serverNames, counter - batchSize,
					counter);
			counter = counter - batchSize;
			mgr.clear();
		...
		// if(!hasNext())
		// throw new
		// IllegalStateException("no more elements, check hasNext first");
