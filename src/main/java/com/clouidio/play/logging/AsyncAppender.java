...
public class AsyncAppender extends AppenderBase<ILoggingEvent> implements
	...
	public void addAppender(Appender<ILoggingEvent> newAppender) {
		if (appenderCount == 0) {
			...
			addWarn("One and only one appender may be attached to AsyncAppender name=["
					+ getName() + "].");
	...
	public void start() {
		...
		LinkedBlockingQueue<Runnable> queue = new LinkedBlockingQueue<Runnable>(
				backupLength);
		exec = new ThreadPoolExecutor(1, 1, 0, TimeUnit.MILLISECONDS, queue,
				handler);
