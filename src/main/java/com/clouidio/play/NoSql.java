...
public class NoSql {
	...
	static {
		try {
			Class<?> c = Class.forName("com.clouidio.play2.Play2Plugin");
			if (c != null) {
	...
	static NoSql get() {
		if (local.get() == null) {
			throw new RuntimeException(
					"The Playorm context is not initialized. NoSqlEntityManager automatically start when one or more classes annotated with the @NoSqlEntity annotation are found in the application.");
		}
		return local.get();
	}
	...
	public static NoSqlEntityManager em() {
		if (plugin2InClassPath) {
			Class<?> clazz;
			try {
				clazz = Class.forName("com.clouidio.play2.NoSqlForPlay2");
				...
				throw new RuntimeException(
						"The play2 class can't be instantiated");
			} catch (IllegalAccessException e) {
				throw new RuntimeException(
						"The play2 plugin class cannot be access");
			}
		} else
	...
	/*
	 * Build a new entityManager. (In most case you want to use the local
