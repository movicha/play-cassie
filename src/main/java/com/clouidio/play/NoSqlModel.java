...
public class NoSqlModel {
	...
	public static <T> T create(ParamNode rootParamNode, String name,
			Class<T> type, Annotation[] annotations) {
		try {
	...
	public static <T> T edit(ParamNode rootParamNode, String name, T o,
			Annotation[] annotations) {
		ParamNode paramNode = rootParamNode.getChild(name, true);
		// #1195 - Needs to keep track of whick keys we remove so that we can
		// restore it before
	...
	private static void processField(Field field, ParamNode paramNode,
			List<RemovedNode> removedNodesList, Object o) {
		...
		//
		if (field.isAnnotationPresent(NoSqlOneToOne.class)
				|| field.isAnnotationPresent(NoSqlManyToOne.class)) {
			isEntity = true;
			relation = field.getType();
		} else if (field.isAnnotationPresent(NoSqlOneToMany.class)
				|| field.isAnnotationPresent(NoSqlManyToMany.class)) {
		...
		if (!isEntity)
			...
			return; // we don't do multiple collections yet(like editing in a
					// table)
		...
		if (theIdStr != null) {
			...
			Validation.addError(fieldParamNode.getOriginalKey(),
					"validation.notFound", theIdStr);
