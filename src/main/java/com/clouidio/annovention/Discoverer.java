 ...
 */
package com.clouidio.annovention;
...
import com.clouidio.annovention.listener.ClassAnnotationDiscoveryListener;
import com.clouidio.annovention.listener.FieldAnnotationDiscoveryListener;
import com.clouidio.annovention.listener.MethodAnnotationDiscoveryListener;
import com.clouidio.annovention.resource.ClassFileIterator;
import com.clouidio.annovention.resource.JarFileIterator;
import com.clouidio.annovention.resource.ResourceIterator;

/**
 * Base annotation discoverer.
 * 
 * @author Kamal Natesan
 */
public abstract class Discoverer {
	 ...
	 * @param listener
	 */
	public final void addAnnotationListener(
			ClassAnnotationDiscoveryListener listener) {
		addAnnotationListener(classAnnotationListeners, listener,
				listener.supportedAnnotations());
	}
	 ...
	 */
	public final void addAnnotationListener(
			FieldAnnotationDiscoveryListener listener) {
		addAnnotationListener(fieldAnnotationListeners, listener,
				listener.supportedAnnotations());
	}
	 ...
	 */
	public final void addAnnotationListener(
			MethodAnnotationDiscoveryListener listener) {
		addAnnotationListener(methodAnnotationListeners, listener,
				listener.supportedAnnotations());
	}
	 ...
	 * @param annotations
	 */
	private <L> void addAnnotationListener(Map<String, Set<L>> map, L listener,
			String... annotations) {
		...
		if (null == annotations || annotations.length == 0) {
			throw new IllegalArgumentException(
					listener.getClass()
							+ " has no supporting Annotations. Check method supportedAnnotations");
		}
	...
	public final void discover(ClassLoader cl) {
		...
		for (URL resource : resources) {
			...
			try {
				ResourceIterator itr = getResourceIterator(resource,
						getFilter());
	...
	private void discoverAndIntimateForClassAnnotations(ClassFile classFile) {
		...
		for (Annotation annotation : annotations) {
			Set<ClassAnnotationDiscoveryListener> listeners = classAnnotationListeners
					.get(annotation.getTypeName());
			if (null == listeners) {
				continue;
			}

			for (ClassAnnotationDiscoveryListener listener : listeners) {
				listener.discovered(classFile.getName(),
	...
	private void discoverAndIntimateForFieldAnnotations(ClassFile classFile) {
		...
		for (FieldInfo fieldInfo : fields) {
			...
			AnnotationsAttribute visible = (AnnotationsAttribute) fieldInfo
					.getAttribute(AnnotationsAttribute.visibleTag);
			AnnotationsAttribute invisible = (AnnotationsAttribute) fieldInfo
					.getAttribute(AnnotationsAttribute.invisibleTag);
			...
			// now tell listeners
			for (Annotation annotation : annotations) {
				Set<FieldAnnotationDiscoveryListener> listeners = fieldAnnotationListeners
						.get(annotation.getTypeName());
				if (null == listeners) {
					continue;
				}
				...
				for (FieldAnnotationDiscoveryListener listener : listeners) {
					listener.discovered(classFile.getName(),
							fieldInfo.getName(), annotation.getTypeName());
				}
			}
		}
	}
	...
	private void discoverAndIntimateForMethodAnnotations(ClassFile classFile) {
		...
		for (MethodInfo methodInfo : methods) {
			...
			AnnotationsAttribute visible = (AnnotationsAttribute) methodInfo
					.getAttribute(AnnotationsAttribute.visibleTag);
			AnnotationsAttribute invisible = (AnnotationsAttribute) methodInfo
					.getAttribute(AnnotationsAttribute.invisibleTag);
			...
			// now tell listeners
			for (Annotation annotation : annotations) {
				Set<MethodAnnotationDiscoveryListener> listeners = methodAnnotationListeners
						.get(annotation.getTypeName());
				if (null == listeners) {
					continue;
				}
	 ...
	 * @throws IOException
	 */
	private ResourceIterator getResourceIterator(URL resource, Filter filter)
			throws IOException {
		URL url = resource;
		...
		if (urlString.endsWith("!/")) {
			...
			if (f.isDirectory()) {
				...
				throw new RuntimeException("bug, how is this not a directory="
						+ f.getAbsolutePath());
			}
		} else {
