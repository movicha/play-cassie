...
public class ClasspathDiscoverer extends Discoverer {
	...
	private static final Logger log = LoggerFactory
			.getLogger(ClasspathDiscoverer.class);
	...
	public final URL[] findResources(ClassLoader cl) {
		...
		while (resources.hasMoreElements()) {
			...
			urlsToScan.add(url);
			log.trace("found url with nosql.Persistence.class so scanning enabled on this url="
					+ url);
		}

		if (urlsToScan.size() == 0)
			throw new IllegalArgumentException(
					"We did not fine the nosql.Persistence.class file on the classpath for any jar.  This file needs to exist in the jars with @Entity objects and we will only scan those jars(or folders)");

		// At this point, we have urls that point to NoSqlPersistence.class
		// which may be in a jar or in
		// a folder.
		for (URL fileInJarOrFolderUrl : urlsToScan) {
			...
			if ("file".equals(protocol)) {
				...
				throw new RuntimeException(
						"protocol of="
								+ protocol
								+ " is not supported for loading classfiles, let me know and I can fix that");
		}
	...
	private void processJar(URL fileInJarOrFolderUrl, List<URL> list) {
		...
		// format of jar file is
		// file:/Users/502065961/Development/play/aaroot/area1/fullSDI/restApi/lib/nosqlorm.jar!/nosql/Persistence.class
		// so we split on ! first nd get the first piece.
		String[] pieces = file.split("!");
		String firstPiece = pieces[0];

		// Now we have firstPiece =
		// file:/Users/502065961/Development/play/aaroot/area1/fullSDI/restApi/lib/nosqlorm.jar
	...
	private URL toUrl(File f) {
		try {
			...
			throw new RuntimeException("bug in converting file to url="
					+ f.getAbsolutePath(), e);
		}
	}
	...
	private Enumeration<URL> loadPersistenceFiles(ClassLoader cl) {
		try {
			Enumeration<URL> resources = cl
					.getResources("nosql/Persistence.class");
			// grrrrr, temporary hack as ecipse can't take two Persistence.java
			// one in src/main/java and one in src/main/test
			// while a JVM can do just fine with that.
			Enumeration<URL> xmlFiles = cl
					.getResources("nosql/Persistence2.class");
	...
	/* @see com.clouidio.annovention.Discoverer#getFilter() */
