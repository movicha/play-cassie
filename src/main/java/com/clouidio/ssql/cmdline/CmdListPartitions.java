...
public class CmdListPartitions {
	...
	private void listPartitions(NoSqlEntityManager mgr, String colFamily,
		...
		if (meta == null)
			throw new InvalidCommand("This column family='" + colFamily
					+ "' does not exist");
		...
		if (partitionedCols.size() == 0) {
			...
			if (partitionedCols.size() > 1)
				throw new InvalidCommand(
						"You must supply a column as this table is partitioned by multiple columns");
		...
		throw new InvalidCommand("ColumnFamily=" + colFamily
				+ " is not partitioned by column='" + field + "'");
	}

	private void listByPartition(DboColumnMeta colMeta, NoSqlEntityManager mgr) {
		if (!(colMeta instanceof DboColumnToOneMeta))
			throw new InvalidCommand(
					"We can only list the partitions when partitioning by a field with @ManyToOne annotation and col="
		...
		String firstPart = "/" + cf + "/<AnyIndexedColumn>/"
				+ fkMeta.getColumnName() + "/";

		System.out.println("Printing partitions of " + cf + " by column "
				+ colMeta.getColumnName());
		System.out.println("");
