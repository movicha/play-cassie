...
public class CmdIndex {
	...
	public void reindex(String cmd, NoSqlEntityManager mgr) {
		...
		System.out.println("row key type="
				+ meta.getIdColumnMeta().getStorageType());
		System.out
				.println("It is safe to kill this process at any time since it only removes duplicates");
		...
		while (true) {
			Map<Object, KeyValue<TypedRow>> keyToRow = findNextSetOfData(s, cf,
					indexView);
			...
			if (rowCountProcessed % 1000 == 0) {
				System.out.println("#Rows processed=" + rowCountProcessed
						+ " totalRows to process=" + totalRows
						+ " totalChanges so far=" + totalChanges);
			}
		}

		System.out.println("#Rows processed=" + rowCountProcessed
				+ " totalRows to process=" + totalRows + " totalChanges="
				+ totalChanges);
	}

	private Counter processAllColumns(NoSqlTypedSession s, ColFamilyData data,
			Map<Object, KeyValue<TypedRow>> keyToRow,
		...
		while (indexView2.next()) {
			...
			if (row == null) {
				...
				// one batch may not have any of the keys of the other batch.
				// This is very normal
			...
			if (rowCounter % 20000 == 0) {
				System.out.println("reindexing.  row count so far="
						+ rowCounter + " num index points changed="
						+ changedCounter);
			}
		}
	...
	private boolean processColumn(NoSqlTypedSession s, ColFamilyData data,
			TypedRow typedRow, IndexPoint pt) {
		String colName = data.getColumn();
		...
		if (column == null) {
			...
			if (!valuesEqual(pt.getIndexedValue(), value)) {
				System.out.println("Entity with rowkey=" + pt.getKeyAsString()
						...
						+ " correct value should be= " + value);
				s.removeIndexPoint(pt, data.getPartitionBy(),
						data.getPartitionId());
				...
				IndexPoint newPoint = new IndexPoint(pt.getRowKeyMeta(), col,
						data.getColumnMeta());
				s.addIndexPoint(newPoint, data.getPartitionBy(),
						data.getPartitionId());
				return true;
			}
		}
	...
	private void removeIndexPt(NoSqlTypedSession s, ColFamilyData data,
			IndexPoint pt) {
		System.out.println("Entity with rowkey=" + pt.getKeyAsString()
				+ " does not exist, WILL remove from index");
	...
	private Map<Object, KeyValue<TypedRow>> findNextSetOfData(
		...
		Cursor<KeyValue<TypedRow>> cursor = s.createFindCursor(cf, keys,
				BATCH_SIZE);
		while (cursor.next()) {
	...
	private ColFamilyData goMore(NoSqlEntityManager mgr, String cf,
			String lastPart) {
		...
		if (meta == null) {
			...
			QueryResult result = mgr.getTypedSession().createQueryCursor(
					"select * from DboTableMeta", 100);
		...
		if (colMeta == null) {
			...
			if (!(colMeta != null && colMeta.getColumnName().equals(field))) {
				System.out.println("Column= " + field + " not found on table "
						+ cf);
				...
				System.out.println("");
				throw new InvalidCommand("Column= " + field
						+ " not found on table " + cf);
			}
		}
