...
public class PlayCassie {

	private static final Logger log = LoggerFactory.getLogger(PlayCassie.class);
	...
	public static void main(String[] args) {
		...
		if (storeType == DbTypeEnum.CASSANDRA) {
			Bootstrap.createAndAddBestCassandraConfiguration(properties, "",
					bean.getKeyspace(), bean.getSeeds());
		} else if (storeType == DbTypeEnum.MONGODB) {
			Bootstrap.createAndAddBestMongoDbConfiguration(properties, "",
					bean.getKeyspace(), bean.getSeeds());
		}
		properties.put(Bootstrap.AUTO_CREATE_KEY, "create");

		NoSqlEntityManagerFactory factory = Bootstrap.create(storeType,
				properties);
	...
	private void start() {
		System.out.println("Welcome to PlayCassie Command Line");
		...
		try {
			...
			while (line != null) {
				prompt = (inCompoundStatement) ? "...\t" : "playcassie >>";
	...
	private void processCommand(String cmd) {
		...
		if ("help".equals(cmd)) {
			...
			processCreate(cmd);
		} else if (startsWithIgnoreCase("SELECT ", cmd)
				|| startsWithIgnoreCase("PARTITIONS ", cmd)) {
